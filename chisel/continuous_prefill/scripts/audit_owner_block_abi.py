...
def audit(out:Path) -> dict:
    ...
    require(fixture['descriptors']==215 and fixture['commands']==21,'fixed scoped table counts')
    require(not any(raw[21*16:]) and not any(db[215*16:]),'nonzero table padding')
    records={i:DescriptorRecord.unpack(int.from_bytes(db[i*16:i*16+16],'little')) for i in range(215)}
    ...
    for pc,op in enumerate(fixture['schedule']):
        chunk=raw[pc*16:pc*16+16];c=Command128.from_bytes(chunk)
        engine=Engine.MATRIX if pc in dims else (Engine.KV if pc==9 else Engine.SFU_CGRA)
        ...
        for index,name in zip((c.src0,c.src1,c.dst),(op['a'],op['b'],op['dst'])):
            if index==NULL_INDEX:require(pc==11 and name is None,'null operand');chains.append([]);continue
            ...
            if index==c.src0 and pc!=9:types+=([RecordType.MATRIX_OP,RecordType.MATRIX_AUX] if pc in dims else [RecordType.SFU_PROGRAM])
        ...
        if pc in dims:
            m,n,k=dims[pc];payload=m|(n<<16)|(k<<32)|(int(pc==10)<<59)
            ...
            expected=SfuProgram(int(c.opcode),1 if pc==11 else 2,input_dtype=TensorDType.FP32,output_dtype=TensorDType.FP32)
    ...
    require(seen==set(range(215)),'unused or missing descriptor record')
    ...
    result={'status':'PASS_OWNER_PUBLIC_ABI_AND_NUMERICAL_RECHECK','commands':commands,'descriptor_records':215,
            ...
            'attention_fusion':[10,11,12],'official_weights':False,'full_model_q1024':False,
