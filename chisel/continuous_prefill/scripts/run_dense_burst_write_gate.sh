...
if [[ -n ${OFFLINE_TOOLS:-} ]];then
 ...
 java -Xmx2500m -XX:ActiveProcessorCount=3 -cp "$OUT/testclasses:$OUT/classes:$CP" heteronpu.continuous.EmitStreamingDenseProbe "$OUT/generated" 1 >"$OUT/emit.log" 2>&1
else
 (cd "$P";sbt -batch compile "Test / runMain heteronpu.continuous.EmitStreamingDenseProbe $OUT/generated 1") >"$OUT/compile_emit.log" 2>&1
fi
