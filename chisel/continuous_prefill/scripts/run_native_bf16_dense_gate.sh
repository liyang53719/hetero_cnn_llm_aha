...
if [[ -n ${OFFLINE_TOOLS:-} ]];then
 ...
 java -Xmx2500m -XX:ActiveProcessorCount=3 -cp "$OUT/testclasses:$OUT/classes:$CP" heteronpu.continuous.EmitStreamingDenseProbe "$OUT/generated" >"$OUT/emit.log" 2>&1
else
 (cd "$P";sbt -batch compile "Test / runMain heteronpu.continuous.EmitStreamingDenseProbe $OUT/generated") >"$OUT/compile_emit.log" 2>&1
fi
...
verilator --cc --exe --assert -Wno-fatal --top-module StreamingDenseProbe -CFLAGS '-O3 -std=c++17 -ffp-contract=off -fno-fast-math' -j "${BUILD_JOBS:-2}" --Mdir "$OUT/obj" --hierarchical "$P/tests/matrix4096_hierarchy.vlt" "${RETAINED_SOURCES[@]}" -f "$OUT/idma.f" "$OUT/generated/StreamingDenseProbe.sv" "$ROOT/rtl/integration/idma_backend_rw_axi_flat_wrap.sv" "$P/tests/native_bf16_dense.cpp" >"$OUT/build.log" 2>&1
...
python3 "$P/scripts/verify_native_bf16_dense.py" "$OUT" >"$OUT/verification.log"
