#!/usr/bin/env bash
# Actual production shared-arbiter + pinned-iDMA protocol/data regression.
set -euo pipefail
...
if [[ -n ${OFFLINE_TOOLS:-} ]]; then
  ...
  java -Xmx1500m -XX:ActiveProcessorCount=2 -cp "$CP" scala.tools.nsc.Main -classpath "$OUT/classes:$CP" \
    ...
    -d "$OUT/testclasses" "$P/src/test/scala/heteronpu/continuous/SharedIdmaWeightBurstProbe.scala" >"$OUT/probe_compile.log" 2>&1
  java -Xmx2G -XX:ActiveProcessorCount=2 -cp "$OUT/testclasses:$OUT/classes:$CP" \
    heteronpu.continuous.EmitSharedIdmaWeightBurstProbe "$OUT/generated" >"$OUT/emit.log" 2>&1
else
  (cd "$P";sbt -batch "Test / runMain heteronpu.continuous.EmitSharedIdmaWeightBurstProbe $OUT/generated") >"$OUT/compile_emit.log" 2>&1
fi
