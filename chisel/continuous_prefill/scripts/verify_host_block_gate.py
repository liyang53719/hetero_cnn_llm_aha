...
def verify(out:Path,write_report:bool=True)->dict:
    ...
    f=json.loads((out/'fixture/manifest.json').read_text());H,F,G,J,D=(f['shape'][x] for x in ['H','F','HEADS','KVHEADS','HD']);T=f['tokens'];K=J*D
    ...
    require([x['opcode'].lower() for x in f['schedule']]==original,'not original 21-op schedule')
    ...
    require(len(cb)==384 and len(db)==((f['descriptors']*16+63)//64)*64,'metadata length')
    ...
    require([int(x['pc']) for x in completions]==list(range(21)),'missing/duplicated/reordered completions')
    ...
    require(values==T*(10*H+7*K+3*F),'independent output count mismatch')
    ...
    for k,v in dict(tokens=T,hidden=H,ffn=F,host_commands=21,completed=21,owner_jobs=19,matrix_commands=9,sfu_commands=11,kv_commands=1,
        checked_fp32=values,bit_differences=0,useful_macs=mac,executed_macs=executed,metadata_reads=21+f['descriptors'],write_ack_bytes=values*4,
    ...
    # Additional checks use actual producer dumps, not only oracle==actual.
    require(data['cache_k']==data['k'] and data['cache_v']==data['v'],'KV copy continuity')
    rawx=(td/'input_x.f32le').read_bytes();require(len(rawx)==T*H*4,'input size')
    for dst,left,right in [('r',rawx,data['o']),('y',data['r'],data['down'])]:
        expect=b''.join(struct.pack('<f',a[0]+b[0]) for a,b in zip(struct.iter_unpack('<f',left),struct.iter_unpack('<f',right)))
        require(data[dst]==expect,'actual-producer residual '+dst)
    h=1469598103934665603
    for (u,) in struct.iter_unpack('<I',data['y']):h=((h^u)*1099511628211)&((1<<64)-1)
    ...
    report={'schema':1,'status':'PASS_HOST_DRIVEN_QWEN2_OWNER_BLOCK','shape':f['shape'],'tokens':T,'commands':21,'owner_jobs':19,'outputs':details,
        ...
        'full_21_record_original_gguf_image':False,'attention_streaming_fusion':[10,11,12],'cold_contiguous_kv_only':True,'synthetic_weights':True,
        'official_model_quality':False,'multilayer':False,'full_network_q1024':False,'dc':False}}
    if write_report:
