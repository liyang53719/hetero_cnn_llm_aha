...
class BlockScalarFloat extends Module {
  ...
  val idle::normal::divIssue::divWait::expScale::expFloor::expFraction::expMul::expAdd::expFinish::done::Nil=Enum(11)
  ...
  switch(state) {
    ...
    is(expFinish){alu.io.op:=true.B;alu.io.x:=horner;alu.io.y:=Cat(0.U(1.W),(127.U(8.W)-k),0.U(23.W));out:=alu.io.out;state:=done}
