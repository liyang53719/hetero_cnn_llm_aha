...
/** Only Host tables launch this root. No legacy block launch, phase permit,
  ...
  */
class HostBlockTop(s:QwenBlockShape, weightReadBeats:Int=1,pipelined:Boolean=false,burstWrites:Boolean=false,commitTailRead:Boolean=false) extends Module {
  ...
  dontTouch(io)
  val cmd=Module(new HostBlockCommands(s,bf16Weights=pipelined));val owner=Module(new QwenOwnerKernel(s,pipelined,burstWrites))
...
class HostBlockCollection(s:QwenBlockShape, weightReadBeats:Int=1,pipelined:Boolean=false,burstWrites:Boolean=false,commitTailRead:Boolean=false) extends Module {
  val top=Module(new HostBlockTop(s,weightReadBeats,pipelined,burstWrites,commitTailRead));val port=IO(chiselTypeOf(top.io));port<>top.io;dontTouch(port)
...
object EmitHostBlock extends App {
  require((args.length>=2&&args.length<=7) && Set("tiny","real").contains(args(1)),"OUT tiny|real [512|4096] [weightReadBeats=1|16] [pipeline=0|1] [burstWrites=0|1] [commitTailRead=0|1]")
  ...
  Files.writeString(out.resolve("HostBlockTop.sv"),ChiselStage.emitSystemVerilog(new HostBlockCollection(s,weightReadBeats,pipelined,burstWrites,commitTailRead),firtoolOpts=Array("--preserve-values=all","-disable-all-randomization")))
