...
/** Only Host tables launch this root. No legacy block launch, phase permit,
  ...
  */
class HostBlockTop(s:QwenBlockShape, weightReadBeats:Int=1) extends Module {
  ...
  if(weightReadBeats==1){
    ...
    io.idmaReadBursts:=dma.io.readBeats;io.idmaReadBeats:=dma.io.readBeats;io.idmaCacheHits:=0.U
  }else{
    val dma=Module(new RetainedIdmaWeightBurstAdapter(weightReadBeats))
...
class HostBlockCollection(s:QwenBlockShape, weightReadBeats:Int=1) extends Module {
  val top=Module(new HostBlockTop(s,weightReadBeats));val port=IO(chiselTypeOf(top.io));port<>top.io;dontTouch(port)
...
object EmitHostBlock extends App {
  require((args.length>=2&&args.length<=4) && Set("tiny","real").contains(args(1)),"OUT tiny|real [512|4096] [weightReadBeats=1|16]")
  ...
  Files.writeString(out.resolve("HostBlockTop.sv"),ChiselStage.emitSystemVerilog(new HostBlockCollection(s,weightReadBeats),firtoolOpts=Array("--preserve-values=all","-disable-all-randomization")))
