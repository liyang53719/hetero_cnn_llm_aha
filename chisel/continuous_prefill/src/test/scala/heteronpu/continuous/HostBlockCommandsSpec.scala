...
class HostBlockCommandsSpec extends AnyFlatSpec with ChiselScalatestTester with Matchers {
  ...
  val commands=(0 until 21).map(word(commandBytes,_)).toVector
  ...
  def initialize(d:HostBlockCommands):Unit={
    ...
    l.commandBase.poke(integer("commandBase").U);l.commandLimit.poke(integer("commandLimit").U);l.commands.poke(21.U)
  ...
  def run(d:HostBlockCommands,cs:Vector[BigInt]=commands,ds:Vector[BigInt]=descriptors,
    ...
    while(!d.io.result.valid.peek().litToBoolean && ticks<5000){
      if(d.io.memory.valid.peek().litToBoolean){
        ...
        d.io.job.bits.dst.expect(destination.U);d.io.job.bits.m.expect(16.U)
        if(pc==12){d.io.job.bits.kind.expect(QwenOwnerKind.Attention.U);success shouldBe 10}
    ...
    if(status==0){memory shouldBe 236;completionError shouldBe false}else{completionError shouldBe true;d.io.resetRequired.expect(true.B)}
    (status,jobs,success)
  }
  def dut=new HostBlockCommands(QwenBlockShape(64,128,2,1,32,1024,true))
  "HostBlockCommands" should "stay idle without Host launch, decode all 21 commands and delay every completion until owner ACK" in {
    test(dut).withAnnotations(Seq(VerilatorBackendAnnotation)){d=>initialize(d);d.clock.step(30);d.io.memory.valid.expect(false.B);d.io.job.valid.expect(false.B);run(d) shouldBe (0,19,21)}
  }
