...
class IdmaCommitTailProbe(maxReadBeats:Int=16) extends Module {
  ...
  val d=Module(new RetainedIdmaWeightBurstAdapter(maxReadBeats,streaming=true,commitTailRead=true))
...
object EmitIdmaCommitTailProbe extends App {
  ...
  Files.writeString(p.resolve("IdmaCommitTailProbe.sv"),ChiselStage.emitSystemVerilog(new IdmaCommitTailProbe(if(args.length>1)args(1).toInt else 16),firtoolOpts=Array("-disable-all-randomization","-strip-debug-info")))
}
