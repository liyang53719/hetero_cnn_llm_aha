...
/** Test wrapper only: real production Dense feeder, eight arithmetic slices,
  ...
  */
class StreamingDenseProbe(burstWrites:Boolean=false,commitTailRead:Boolean=false,coalesce:Boolean=true,maxK:Int=128) extends Module {
  val io=IO(new Bundle {
    val job=Flipped(Decoupled(new QwenOwnerJob));val done=Decoupled(new QwenOwnerResult)
    val axi=new BlockAxiMaster;val resetRequired=Output(Bool());val profile=Output(new DenseCycleProfile)
    val physicalSteps=Output(UInt(64.W));val wideSteps=Output(UInt(64.W));val streamedBeats=Output(UInt(64.W))
  })
  val dense=Module(new StreamingDenseOwner(maxK,burstWrites=burstWrites,coalesceWeightReads=coalesce));val matrix=Module(new MatrixPipelineService)
  ...
  io.profile:=dense.io.profile;io.physicalSteps:=matrix.io.acceptedSteps;io.wideSteps:=matrix.io.wideSteps;io.streamedBeats:=dma.io.streamedBeats
  dontTouch(io)
}
class StreamingDenseCollection(burstWrites:Boolean=false,commitTailRead:Boolean=false,coalesce:Boolean=true,maxK:Int=128) extends Module {
  val top=Module(new StreamingDenseProbe(burstWrites,commitTailRead,coalesce,maxK));val port=IO(chiselTypeOf(top.io));port<>top.io;dontTouch(port)
...
object EmitStreamingDenseProbe extends App {
  ...
  Files.writeString(p.resolve("StreamingDenseProbe.sv"),ChiselStage.emitSystemVerilog(new StreamingDenseCollection(args.length>1 && args(1)=="1",args.length>2 && args(2)=="1",args.length<=3||args(3)!="0",if(args.length>4)args(4).toInt else 128),firtoolOpts=Array("-disable-all-randomization","-strip-debug-info")))
}
