...
/** Test wrapper only: real production Dense feeder, eight arithmetic slices,
  ...
  */
class StreamingDenseProbe extends Module {
  ...
  val dense=Module(new StreamingDenseOwner(128));val matrix=Module(new MatrixPipelineService)
  val dma=Module(new RetainedIdmaWeightBurstAdapter(16,streaming=true))
...
class StreamingDenseCollection extends Module {
  val top=Module(new StreamingDenseProbe);val port=IO(chiselTypeOf(top.io));port<>top.io;dontTouch(port)
...
object EmitStreamingDenseProbe extends App {
  ...
  Files.writeString(p.resolve("StreamingDenseProbe.sv"),ChiselStage.emitSystemVerilog(new StreamingDenseCollection,firtoolOpts=Array("-disable-all-randomization","-strip-debug-info")))
}
