...
// new tests change only B storage and M tiling, not the memory service timing.
...
struct Test{
 VStreamingDenseProbe d;std::vector<uint32_t> memory;Beat pending,aw,w;uint32_t rng=9082026;uint64_t cycles=0,reads=0,writes=0,readBursts=0;unsigned m=0,n=0,k=0;bool running=false,injected=false;int fault=0;bool native=false;std::ofstream actual,reference;
 ...
 uint64_t run(unsigned mm,unsigned nn,unsigned kk,unsigned seed,int fail=0,bool bf16=false){
  ck(!running&&!pending.valid&&!aw.valid&&!w.valid,"undrained previous request");m=mm;n=nn;k=kk;native=bf16;fault=fail;injected=false;cycles=reads=writes=readBursts=0;
  ...
  std::cout<<"STREAM_DENSE_CASE_PASS m="<<m<<" n="<<n<<" k="<<k<<" bf16="<<native<<" fault="<<fail<<" checked_fp32="<<checked<<" cycles="<<cycles<<" read_beats="<<reads<<" read_bursts="<<readBursts<<" write_acks="<<writes<<std::endl;return checked;
 }
};
int main(int argc,char**argv){try{
 ...
 std::cout<<"NATIVE_DENSE_NUMERIC_PASS cases=13 numeric_cases=11 fault_resets=2 checked_fp32="<<count<<" bit_differences=0 real_idma=1 physical_mac=4096 same_dut=1\n";return 0;
