...
static void need(bool b,const char* msg){if(!b)throw std::runtime_error(msg);}
...
public:
  ...
  bool running=false;
  uint64_t visibleWriteBytes=0;
  std::array<uint64_t,15> stageWriteBeats{},stageReadBeats{},stageTraceHash{};
  void openEvidence(){
    const char* value=std::getenv("QWEN_BLOCK_EVIDENCE_DIR");
    if(!value)return;
    evidenceDir=value;
    need(!std::filesystem::exists(evidenceDir),"refuse to overwrite earlier tensor evidence");
    need(std::filesystem::create_directories(evidenceDir),"cannot create tensor evidence directory");
  }
  void saveTensor(const std::string& name,const uint32_t* data,size_t count){
    if(evidenceDir.empty())return;
    std::ofstream file(evidenceDir/(name+".f32le"),std::ios::binary);
    need(bool(file),"cannot create tensor dump");
    file.write(reinterpret_cast<const char*>(data),count*4);
    need(bool(file),"failed to save complete tensor dump");
  }
  void checkWrite(uint64_t off,uint64_t mask){
    need(commits<15,"write after final stage");
    const auto& stage=STAGES[commits];
    need(off>=stage.off && off+64<=stage.off+size_t(tokens)*stage.width*4,
      "write outside active stage output or into guard region");
    need(mask==UINT64_MAX,"all Qwen2 block dimensions require complete FP32 beats");
    for(unsigned i=0;i<16;i++)need(!initialized[off/4+i],"duplicate overwrite of produced tensor");
  }
  void commitWrite(const Pending& p){
    auto off=p.address-BASE; checkWrite(off,p.mask);
    // Same 64-byte DDR write semantics, without per-byte string allocations.
    for(unsigned i=0;i<16;i++){
      memory[off/4+i]=p.data[i]; initialized[off/4+i]=1;
      stageTraceHash[commits]=(stageTraceHash[commits]^p.data[i])*1099511628211ULL;
    }
    stageTraceHash[commits]=(stageTraceHash[commits]^off)*1099511628211ULL;
    stageWriteBeats[commits]++; visibleWriteBytes+=64;
  }
  void guardCheck(){
    for(const auto& stage:STAGES){
      size_t begin=stage.off/4+size_t(tokens)*stage.width,end=stage.off/4+size_t(MAX_TOKENS)*stage.width;
      for(size_t i=begin;i<end;i++)need(memory[i]==0x7fc00001u && !initialized[i],"unused output guard changed");
    }
    std::cout<<"DDR_GUARD_PASS visible_write_bytes="<<visibleWriteBytes<<" host_intermediate_writes=0"<<std::endl;
  }
  ...
  void set(uint64_t offset,size_t i,float x){need(!running && offset<WRITABLE_START,"host writes forbidden during active request");memory[offset/4+i]=bits(x);reference[offset/4+i]=x;initialized[offset/4+i]=1;}
  ...
  void initialize(unsigned epoch=1){
    ...
    openEvidence();
    saveTensor("input_x",memory.data()+OFF_X/4,size_t(tokens)*H);
    computeReference();
  }
  ...
  void compareStage(unsigned stage){
    ...
    need(stageWriteBeats[stage]*16==count,"stage did not receive exact number of write ACKs");
    saveTensor("phase_"+std::to_string(stage)+"_"+s.name+"_actual",memory.data()+s.off/4,count);
    std::vector<uint32_t> expected(count);for(size_t i=0;i<count;i++)expected[i]=bits(get(s.off,i));
    saveTensor("phase_"+std::to_string(stage)+"_"+s.name+"_reference",expected.data(),count);
    std::cout<<"STAGE_MEMORY phase="<<stage<<" reads="<<stageReadBeats[stage]<<" write_acks="<<stageWriteBeats[stage]
      <<" visible_bytes="<<stageWriteBeats[stage]*64<<" write_trace_fnv="<<std::hex<<stageTraceHash[stage]<<std::dec<<std::endl;
    checked+=count;bitDifferences+=mismatch;globalMaxError=std::max(globalMaxError,maxError);published[stage]=true;commits++;
  }
  void checkRead(uint64_t offset){for(int i=0;i<16;i++)if(!initialized[offset/4+i])throw std::runtime_error("read from poison/uninitialized DDR at "+std::to_string(offset));
    if(commits<15)stageReadBeats[commits]++;
  ...
  void step(){
    ...
    if(rf)pending=next;else if(pending.valid&&pending.delay)pending.delay--;d.clock=0; // falling edge is evaluated with next cycle inputs, before its rising edge
  }
#else
  ...
  void step(){
    ...
    d.clock=1;d.eval();cycles++;
    if(ack){if(pending.write&&!pending.error&&!(corruptTag&&injected)){commitWrite(pending);}
      acks++;pending.valid=false;}
    if(next.valid)pending=next;else if(pending.valid&&pending.delay)pending.delay--;d.clock=0; // falling edge is evaluated with next cycle inputs, before its rising edge
  }
#endif
  void nextRequest(unsigned epoch,bool reset){
    ...
    stageWriteBeats.fill(0);stageReadBeats.fill(0);stageTraceHash.fill(0);visibleWriteBytes=0;
