...
int main(int c,char**v){if(c!=7)return 2;try{int batch=std::stoi(v[4]);auto all=read<uint32_t>(v[1],32);auto res=read<uint32_t>(v[2],32);auto w=read<uint16_t>(v[3],16);if(all.size()!=128ull*K||res.size()!=R*N||w.size()!=1ull*K*N)throw std::runtime_error("count");std::vector<float>x(R*K);for(int r=0;r<R;r++)for(int k=0;k<K;k++)x[r*K+k]=ff((uint32_t)bf(ff(all[(batch*R+r)*K+k]))<<16);std::vector<uint32_t>d(R*N),fin(R*N);
...
for(size_t i=0;i<d.size();i++)fin[i]=bb(ff(d[i])+ff(res[i]));write(v[5],d);write(v[6],fin);std::cout<<"L5_Q128_DOWN_CPP_GOLDEN_PASS batch="<<batch<<" rows=16 shape=8960x1536\n";}catch(const std::exception&e){std::cerr<<e.what()<<'\n';return 1;}return 0;}
