# Revision 8B-A H3: retained front/broadcast/cluster/glue plus constrained
# incremental compile of top-level distribution nets only.
...
set OPERAND_DDC [require_env OPERAND_DDC]
...
foreach ddc [list $FRONT_DDC $BROADCAST_DDC $OPERAND_DDC $CLUSTER_DDC $GLUE_DDC] { read_ddc $ddc -scenarios {} }
analyze -format sverilog $TOP_RTL
elaborate bf16_outer_product_context_array_rev8b_a_candidate
current_design bf16_outer_product_context_array_rev8b_a_candidate
set link_status [link]
foreach ref {bf16_context_front_control_rev8_candidate bf16_front_to_cluster_broadcast32_rev8b_a_candidate bf16_operand_distribution512_rev8b_a_candidate bf16_context_lane_cluster16_rev8_candidate bf16_outer_product_array_glue512} {
...
# Every high-fanout source is now an independently mapped retained boundary.
# No top compile: this avoids traversing/optimizing the 2.6M retained lane cells.
...
set broadcast_instances [sizeof_collection [get_cells -hierarchical -quiet -filter "ref_name == bf16_front_to_cluster_broadcast32_rev8b_a_candidate"]]
set operand_instances [sizeof_collection [get_cells -hierarchical -quiet -filter "ref_name == bf16_operand_distribution512_rev8b_a_candidate"]]
...
puts $status_fp "PHASE=revision8b_a_structural_h3"
...
puts $status_fp "BROADCAST32_INSTANCES=$broadcast_instances"
puts $status_fp "OPERAND_DISTRIBUTION_INSTANCES=$operand_instances"
...
if {!$link_status || $unmapped_count != 0 || $front_instances != 1 || $broadcast_instances != 1 || $operand_instances != 1 || $cluster_instances != 32 || $glue_instances != 1} { exit 4 }
puts "L5_REV8B_A_H3_COMPLETED WNS=$worst_slack"
exit
