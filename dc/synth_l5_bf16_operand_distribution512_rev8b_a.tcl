...
set RTL [require_env OPERAND_RTL]
...
elaborate bf16_operand_distribution512_rev8b_a_candidate
current_design bf16_operand_distribution512_rev8b_a_candidate
...
if {$HIGH} {
  foreach pattern {INV_X1M_A6P5PP140ZTS_C35 INV_X2M_A6P5PP140ZTS_C35 BUFHA_X2M_A6P5PP140ZTS_C35} {
    set weak_cells [get_lib_cells -quiet "*/$pattern"]
    if {[sizeof_collection $weak_cells] > 0} { set_dont_use $weak_cells }
  }
...
puts $status_fp "PHASE=revision8b_a_operand_distribution512"
puts $status_fp "REVISION=8B-A"
puts $status_fp "EFFORT=[expr {$HIGH ? "high" : "normal"}]"
...
puts $status_fp "CELL_AREA=[get_attribute [current_design] area]"
...
puts "L5_REV8B_A_OPERAND_DISTRIBUTION_COMPLETED WNS=$worst_slack CELLS=$cell_count"
exit
