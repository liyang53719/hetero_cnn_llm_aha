#!/usr/bin/env python3
"""Install exact sandbox-tested Chisel/test bytes; never overwrite concurrent work."""
...
files = payload['files']
need(len(files) == 18, 'INCOMPLETE_SOURCE_CHANGESET')
for name in files:
...
for attempt in range(3):
    ...
    if all(data is not None and sha(data) == files[name]['sha256'] for name,data in current.items()):
        commit = parent
        break
    materialized = {}
    for name, change in files.items():
        existing = current[name]
        need((None if existing is None else sha(existing)) == change['old_sha256'], 'CONCURRENT_TARGET_CHANGE:' + name)
        original = read(parent,change['source']) if change['source'] else b''
        need(original is not None and sha(original) == change['source_sha256'], 'CONCURRENT_SOURCE_CHANGE:' + name)
        lines = original.decode('utf-8').splitlines(keepends=True)
        previous = len(lines) + 1
        for begin,end,text in reversed(change['edits']):
    ...
    commit = git('commit-tree',tree,'-p',parent,'-m','perf(chisel): batch Dense stores and forward iDMA payload with terminal commit barrier [skip ci]',env=env).decode().strip()
...
report = {'status':'SOURCE_BYTES_PUBLISHED_NOT_FULL_PERFORMANCE_ACCEPTANCE','commit':commit,
          'files':{name:x['sha256'] for name,x in files.items()},'remote_bytes_verified':True,
