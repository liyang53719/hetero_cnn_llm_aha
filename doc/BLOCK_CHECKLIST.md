...
| C06 | P0 | 超64命令与有限tensor存活集合 | to do |
| C06.1 | P0 | 长命令容量与生命周期控制专项测试设计 | to do |
| C06.2 | P0 | 长命令实际frontend接入与全部门禁 | to do |
| C07 | P0 | 有界片上状态tile与事务提交 | to do |
| C08 | P0 | 建立完整block公共测试与回执校验器 | to do |
| C08.1 | P0 | 全量tensor/state回执schema与比较器框架 | to do |
| C08.2 | P0 | 真实block公共runner和全部比较域集成 | to do |
| Q20 | P1 | 重跑当前Qwen2双层主链保护基线 | to do |
| Q21 | P1 | 官方Qwen2 block权重与输入导出 | to do |
| Q21.1 | P1 | 通用权重分块打包与BF16字节回读测试 | to do |
