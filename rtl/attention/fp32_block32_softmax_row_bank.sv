...
module fp32_block32_softmax_row_bank#(parameter integer LANES=8)(
 ...
 input logic[2:0]reduce_level_i,input logic[LANES*4-1:0]reduce_cols_i,output logic[LANES*32-1:0]reduce_x_o,reduce_y_o,input logic[LANES-1:0]reduce_write_valid_i,input logic[LANES*4-1:0]reduce_write_cols_i,input logic[LANES*32-1:0]reduce_results_i,
 ...
 function automatic logic fp32_gt(input logic[31:0]a,input logic[31:0]b);logic[31:0]ka,kb;begin ka=a[31]?~a:{1'b1,a[30:0]};kb=b[31]?~b:{1'b1,b[30:0]};fp32_gt=ka>kb;end endfunction
 ...
 always_comb begin for(integer k=0;k<LANES;k++)begin col=reduce_cols_i[k*4+:4];if(reduce_level_i==0)begin reduce_x_o[k*32+:32]=weight_mem[2*col];reduce_y_o[k*32+:32]=weight_mem[2*col+1];end else begin reduce_x_o[k*32+:32]=reduce_mem[2*col];reduce_y_o[k*32+:32]=reduce_mem[2*col+1];end end end
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)max_q<=32'hff800000;else begin if(start_i)max_q<=32'hff800000;if(load_valid_i)begin score_mem[load_col_i]<=load_mask_i?32'hff800000:load_score_i;if(!load_mask_i&&(max_q==32'hff800000||fp32_gt(load_score_i,max_q)))max_q<=load_score_i;end if(exp_write_valid_i)weight_mem[exp_write_col_i]<=exp_weight_i;for(integer k=0;k<LANES;k++)if(reduce_write_valid_i[k])reduce_mem[reduce_write_cols_i[k*4+:4]]<=reduce_results_i[k*32+:32];end end
endmodule
