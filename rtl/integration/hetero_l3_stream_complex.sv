...
module hetero_l3_stream_complex (
  ...
  input  logic          sfu_select_dedicated_i,
  input  logic          dedicated_cfg_valid_i,
  output logic          dedicated_cfg_ready_o,
  input  logic          dedicated_cfg_op_i,
  input  logic [3:0]    dedicated_cfg_h_i,
  input  logic [3:0]    dedicated_cfg_w_i,
  input  logic [4:0]    dedicated_cfg_c_i,
  input  logic [6:0]    dedicated_cfg_bytes_i,
  input  logic [15:0]   dedicated_cfg_tag_i,
  input  logic [11:0]   dedicated_cfg_tensor_id_i,
  input  logic [3:0]    dedicated_cfg_format_i,
  input  logic          dedicated_secondary_valid_i,
  output logic          dedicated_secondary_ready_o,
  input  logic [511:0]  dedicated_secondary_data_i,
  input  logic [63:0]   dedicated_secondary_be_i,
  input  logic          dedicated_secondary_last_i,
  input  logic [3:0]    dedicated_secondary_format_i,
  output logic          dedicated_transfer_done_o,
  output logic [31:0]   dedicated_protocol_error_count_o,
  output logic [31:0]   sfu_mux_protocol_error_count_o,
  ...
  sfu_stream_endpoint_mux u_sfu_endpoint (
    .clk_i,.rst_ni,.select_dedicated_i(sfu_select_dedicated_i),
    .aha_cfg_valid_i,.aha_cfg_ready_o,.aha_cfg_input_base_i,.aha_cfg_output_base_i,
    .aha_cfg_input_beats_i,.aha_cfg_output_beats_i,.aha_cfg_output_tag_i,
    .aha_cfg_output_tensor_id_i,.aha_cfg_output_format_i,.aha_cfg_output_last_be_i,
    .aha_run_done_i,.aha_proc_packet_wr_en_o,.aha_proc_packet_wr_addr_o,
    .aha_proc_packet_wr_data_o,.aha_proc_packet_wr_strb_o,.aha_proc_packet_rd_en_o,
    .aha_proc_packet_rd_addr_o,.aha_proc_packet_rd_data_i,
    .aha_proc_packet_rd_data_valid_i,.aha_native_eos_o,.aha_transfer_done_o,
    .aha_protocol_error_count_o,.dedicated_cfg_valid_i,.dedicated_cfg_ready_o,
    .dedicated_cfg_op_i,.dedicated_cfg_h_i,.dedicated_cfg_w_i,.dedicated_cfg_c_i,
    .dedicated_cfg_bytes_i,.dedicated_cfg_tag_i,.dedicated_cfg_tensor_id_i,
    .dedicated_cfg_format_i,.secondary_valid_i(dedicated_secondary_valid_i),
    .secondary_ready_o(dedicated_secondary_ready_o),
    .secondary_data_i(dedicated_secondary_data_i),.secondary_be_i(dedicated_secondary_be_i),
    .secondary_last_i(dedicated_secondary_last_i),
    .secondary_format_i(dedicated_secondary_format_i),.dedicated_transfer_done_o,
    .dedicated_protocol_error_count_o,.mux_protocol_error_count_o(sfu_mux_protocol_error_count_o),
    .primary_valid_i(stream_out_valid[0]),.primary_ready_o(aha_in_ready),
    .primary_data_i(stream_out_data[0*512 +:512]),.primary_be_i(stream_out_be[0*64 +:64]),
    .primary_tag_i(stream_out_tag[0*16 +:16]),
    .primary_tensor_id_i(stream_out_tensor[0*12 +:12]),.primary_last_i(stream_out_last[0]),
    .primary_format_i(stream_out_format[0*4 +:4]),.out_valid_o(aha_out_valid),
    .out_ready_i(aha_out_ready),.out_data_o(aha_out_data),.out_be_o(aha_out_be),
    .out_tag_o(aha_out_tag),.out_tensor_id_o(aha_out_tensor),
    .out_last_o(aha_out_last),.out_format_o(aha_out_format)
  );
