...
module operator_matrix_bf16_endpoint_v3 (
  ...
  assign req_ready_o=!active_q&&!pending_q;
  assign step_ready_o=active_q&&matrix_in_ready;
  ...
  assign protocol_error_o=matrix_error;
  bf16_outer_product_context_array_rev8b_b_candidate matrix(
    .clk_i,.rst_ni,.in_valid_i(step_valid_i&&active_q),.in_ready_o(matrix_in_ready),
  ...
  always_ff@(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)begin active_q<=0;pending_q<=0;tag_q<=0;parent_q<=0;terminal_q<=0;status_q<=0;end
      ...
      if(active_q&&matrix_error)begin active_q<=0;status_q<=8'd7;pending_q<=1;end
      else if(out_valid_o&&out_ready_i&&out_last_o)begin active_q<=0;status_q<=0;pending_q<=1;end
