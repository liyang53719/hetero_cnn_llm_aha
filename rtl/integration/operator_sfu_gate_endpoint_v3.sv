...
module operator_sfu_gate_endpoint_v3(
 ...
 localparam logic[2:0]IDLE=0,PAYLOAD=1,ISSUE=2,WAIT=3,COMPLETE=4;logic[2:0]st;logic[15:0]tag_q;logic[7:0]parent_q,terminal_q,status_q;logic[127:0]gate_q,up_q;logic last_q;
 ...
 assign req_ready_o=st==IDLE;assign payload_ready_o=st==PAYLOAD;assign result_valid_o=st==WAIT&&aov;assign result_bf16_o=ao;assign result_last_o=al;assign exception_flags_o=af;
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin st<=IDLE;tag_q<=0;parent_q<=0;terminal_q<=0;status_q<=0;gate_q<=0;up_q<=0;last_q<=0;end else case(st)
