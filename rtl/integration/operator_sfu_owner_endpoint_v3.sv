...
module operator_sfu_owner_endpoint_v3(
 ...
 logic active_q;logic[2:0]sel_q,req_sel;logic[15:0]bad_tag_q;logic[7:0]bad_parent_q,bad_terminal_q;
 ...
 assign req_sel=decode(req_opcode_i);assign req_ready_o=!active_q&&(req_sel==BAD||crdy[req_sel]);
 wire req_fire=req_valid_i&&req_ready_o;

 operator_sfu_vector_endpoint_v3 vec(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==VEC),.req_ready_o(crdy[VEC]),.req_opcode_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,.req_variant_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==VEC),.payload_ready_o(prdy[VEC]),.payload_a_i(payload_a_i),.payload_b_i(payload_b_i),.payload_mask_i(payload_mask_i),
 ...
 operator_sfu_scalar_endpoint_v3 scalar(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==SCALAR),.req_ready_o(crdy[SCALAR]),.req_opcode_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==SCALAR),.payload_ready_o(prdy[SCALAR]),.payload_x_i(payload_a_i[31:0]),.result_valid_o(rv[SCALAR]),.result_ready_i(rr[SCALAR]),.result_y_o(rdata[SCALAR*512+:32]),.exception_flags_o(rflags[SCALAR*13+:13]),.domain_error_o(rde[SCALAR]),
 ...
 operator_sfu_norm_endpoint_v3 norm(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==NORM),.req_ready_o(crdy[NORM]),.req_opcode_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==NORM),.payload_ready_o(prdy[NORM]),.payload_x_i(payload_a_i),.payload_weight_i(payload_b_i),.payload_bias_i(payload_c_i),.payload_epsilon_i,
 ...
 operator_sfu_rope_endpoint_v3 rope(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==ROPE),.req_ready_o(crdy[ROPE]),.req_opcode_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==ROPE),.payload_ready_o(prdy[ROPE]),.payload_data_i(payload_a_i),.payload_trig_i(payload_b_i),
 ...
 operator_sfu_online_softmax_endpoint_v3 soft_ep(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==SOFT),.req_ready_o(crdy[SOFT]),.req_opcode_i,.req_variant_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
 ...
 operator_sfu_gate_endpoint_v3 gate(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==GATE),.req_ready_o(crdy[GATE]),.req_opcode_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==GATE),.payload_ready_o(prdy[GATE]),.payload_gate_bf16_i(payload_a_i[127:0]),.payload_up_bf16_i(payload_b_i[127:0]),.payload_last_i,
 ...
 operator_sfu_pwl_endpoint_v3 pwl(.clk_i,.rst_ni,.req_valid_i(req_fire&&req_sel==PWL),.req_ready_o(crdy[PWL]),.req_opcode_i,.req_variant_i,.req_tag_i,.req_parent_phase_i,.req_terminal_phase_i,
  .payload_valid_i(payload_valid_i&&active_q&&sel_q==PWL),.payload_ready_o(prdy[PWL]),.payload_x_i(payload_a_i),.result_valid_o(rv[PWL]),.result_ready_i(rr[PWL]),.result_y_o(rdata[PWL*512+:512]),.exception_flags_o(rflags[PWL*13+:5]),
 ...
 always_comb begin ccr=0;rr=0;payload_ready_o=0;result_valid_o=0;result_data_o=0;result_last_o=0;exception_flags_o=0;domain_error_o=0;
  if(active_q&&sel_q!=BAD)begin ccr[sel_q]=completion_ready_i;if(sel_q!=SOFT)begin rr[sel_q]=result_ready_i;payload_ready_o=prdy[sel_q];result_valid_o=rv[sel_q];result_data_o=rdata[sel_q*512+:512];result_last_o=rlast[sel_q];exception_flags_o=rflags[sel_q*13+:13];domain_error_o=rde[sel_q];end end
 end
 assign completion_valid_o=active_q&&(sel_q==BAD||ccv[sel_q]);assign completion_tag_o=sel_q==BAD?bad_tag_q:ctag[sel_q*16+:16];assign completion_parent_phase_o=sel_q==BAD?bad_parent_q:cparent[sel_q*8+:8];assign completion_terminal_phase_o=sel_q==BAD?bad_terminal_q:cterminal[sel_q*8+:8];assign completion_status_o=sel_q==BAD?8'd4:cstatus[sel_q*8+:8];
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin active_q<=0;sel_q<=BAD;bad_tag_q<=0;bad_parent_q<=0;bad_terminal_q<=0;end else begin if(req_fire)begin active_q<=1;sel_q<=req_sel;if(req_sel==BAD)begin bad_tag_q<=req_tag_i;bad_parent_q<=req_parent_phase_i;bad_terminal_q<=req_terminal_phase_i;end end if(completion_valid_o&&completion_ready_i)active_q<=0;end end
endmodule
