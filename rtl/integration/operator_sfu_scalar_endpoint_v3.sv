...
module operator_sfu_scalar_endpoint_v3(
 ...
 fp32_reciprocal_nr2 recip(.clk_i,.rst_ni,.in_valid_i(pv),.in_ready_o(pr),.x_i(payload_x_i),.out_valid_o(pov),.out_ready_i(por),.y_o(py),.exception_flags_o(pf),.domain_error_o(pde),.accepted_o(pa),.completed_o(pc));
 fp32_exp2_pwl_pipe exp2(.clk_i,.rst_ni,.in_valid_i(ev),.in_ready_o(er),.x_i(payload_x_i),.out_valid_o(eov),.out_ready_i(eor),.y_o(ey),.exception_flags_o(ef),.accepted_o(ea),.completed_o(ec));
