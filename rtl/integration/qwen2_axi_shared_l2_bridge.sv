...
module qwen2_axi_shared_l2_bridge import hetero_idma_512_pkg::*;#(
 ...
 logic write_response_q,read_pending_q;
 ...
 assign busy_o=converter_busy||read_pending_q;
 always_comb begin
  l2_rd_valid_o=mem_req&&!mem_we&&!read_pending_q;l2_rd_addr_o=ADDR_W'(mem_addr[ADDR_W+5:6]);l2_rsp_ready_o=read_pending_q;
  ...
  if(mem_req)mem_gnt=mem_we?l2_wr_ready_i:(l2_rd_ready_i&&!read_pending_q);
  mem_rvalid=write_response_q||(l2_rsp_valid_i&&read_pending_q);mem_rdata=l2_rsp_data_i;
 end
 always_ff@(posedge clk_i or negedge rst_ni)begin
  if(!rst_ni)begin write_response_q<=1'b0;read_pending_q<=1'b0;end
  else begin write_response_q<=mem_req&&mem_gnt&&mem_we;
   if(mem_req&&mem_gnt&&!mem_we)read_pending_q<=1'b1;
   if(l2_rsp_valid_i&&l2_rsp_ready_o)read_pending_q<=1'b0;
  end
 end
endmodule
