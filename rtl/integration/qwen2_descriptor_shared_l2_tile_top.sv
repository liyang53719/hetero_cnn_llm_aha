...
module qwen2_descriptor_shared_l2_tile_top #(
  ...
  input logic[127:0]rms_command_i,input logic[127:0]matrix_command_i,input logic[5:0]q_column_tile_i,input logic full_q_i,
  ...
  logic plan_context_valid,plan_context_ready,replay_context_q,full_q_q;logic[5:0]tile_q,start_tile_q;
  logic[63:0]plan_ddr_read,plan_ddr_write,agg_ddr_read_q,agg_ddr_write_q;
  logic[31:0]payload_l2_read,payload_l2_write,agg_l2_read_q,agg_l2_write_q;
  ...
  logic scomp_seen_q,mcomp_seen_q,active_q,last_tile;
  assign plan_context_valid=context_valid||replay_context_q;
  assign context_ready=plan_context_ready&&!replay_context_q;
  assign last_tile=!full_q_q||tile_q==6'd47;
  assign ddr_read_bytes_o=agg_ddr_read_q+plan_ddr_read;assign ddr_write_bytes_o=agg_ddr_write_q+plan_ddr_write;
  assign l2_read_beats_o=agg_l2_read_q+payload_l2_read;assign l2_write_beats_o=agg_l2_write_q+payload_l2_write;
  ...
  qwen2_tile_dma_plan plan(.clk_i,.rst_ni,.context_valid_i(plan_context_valid),.context_ready_o(plan_context_ready),
    .context_legal_i(context_legal),.tensor_address_i(addresses),.q_column_tile_i(tile_q),.weight_src_stride_i(32'd3072),.token_index_i(0),.output_token_bytes_i(3072),.full_q_i(full_q_q),
  ...
  qwen2_shared_l2_tile_payload #(.ADDR_W(ADDR_W))payload(.clk_i,.rst_ni,.start_i(payload_start_q),.reuse_norm_i(tile_q!=start_tile_q),.load_norm_i(1'b0),
    ...
    .read_beats_o(payload_l2_read),.write_beats_o(payload_l2_write));
  ...
  qwen2_matrix_command_endpoint matrix(.clk_i,.rst_ni,.cmd_valid_i(m_cmd_pending_q),.cmd_ready_o(m_cmd_ready),
    ...
    .step_clear_i(mpclear),.step_last_i(mplast),.command_last_tile_i(last_tile),.step_a_i(mpa),.step_b_i(mpb),
  ...
  assign done_o=plan_done&&last_tile&&((scomp_seen_q&&mcomp_seen_q)||plan_status!=0);
  always_comb begin
  ...
  always_ff@(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)begin rms_q<=0;matrix_q<=0;context_start_q<=0;store_start_q<=0;payload_start_q<=0;
      ...
      replay_context_q<=0;full_q_q<=0;tile_q<=0;start_tile_q<=0;agg_ddr_read_q<=0;agg_ddr_write_q<=0;agg_l2_read_q<=0;agg_l2_write_q<=0;
    end else begin
      context_start_q<=0;store_start_q<=0;payload_start_q<=0;
      if(replay_context_q&&plan_context_ready)replay_context_q<=0;
      if(start_i&&!active_q)begin rms_q<=rms_command_i;matrix_q<=matrix_command_i;context_start_q<=1;
        s_cmd_pending_q<=1;m_cmd_pending_q<=1;scomp_seen_q<=0;mcomp_seen_q<=0;payload_started_q<=0;active_q<=1;
        full_q_q<=full_q_i;tile_q<=q_column_tile_i;start_tile_q<=q_column_tile_i;agg_ddr_read_q<=0;agg_ddr_write_q<=0;agg_l2_read_q<=0;agg_l2_write_q<=0;end
      ...
      if(plan_done&&full_q_q&&tile_q<47&&plan_status==0)begin
        agg_ddr_read_q<=agg_ddr_read_q+plan_ddr_read;agg_ddr_write_q<=agg_ddr_write_q+plan_ddr_write;
        agg_l2_read_q<=agg_l2_read_q+payload_l2_read;agg_l2_write_q<=agg_l2_write_q+payload_l2_write;
        tile_q<=tile_q+1;replay_context_q<=1;payload_started_q<=0;
      end
