...
module qwen2_matrix_command_endpoint(
 ...
 input logic step_valid_i,output logic step_ready_o,input logic[2:0]step_context_i,input logic step_clear_i,step_last_i,input logic command_last_tile_i,input logic[255:0]step_a_i,input logic[511:0]step_b_i,
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin active_q<=0;pending_q<=0;event_q<=0;status_q<=0;end else begin if(cmd_valid_i&&cmd_ready_o)begin event_q<=cmd_i[55:40];if(cmd_i[7:0]inside{8'h20,8'h21,8'h23,8'h24})begin active_q<=1;status_q<=0;end else begin pending_q<=1;status_q<=8'd4;end end if(out_valid_o&&out_ready_i&&out_last_o&&command_last_tile_i)begin active_q<=0;pending_q<=1;end if(pending_q&&completion_ready_i)pending_q<=0;end end
endmodule
