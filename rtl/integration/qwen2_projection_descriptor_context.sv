...
`timescale 1ns/1ps
module qwen2_projection_descriptor_context(
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin
  if(!rst_ni)begin state_q<=S_IDLE;slot_q<=0;status_q<=OK;shape_index_q<=0;for(seq_c=0;seq_c<3;seq_c++)begin roots_q[seq_c]<=24'hffffff;addr_q[seq_c]<=0;shape_q[seq_c]<=0;end end
  else case(state_q)
   S_IDLE:if(start_i)begin roots_q[0]<=command_i[79:56];roots_q[1]<=command_i[103:80];roots_q[2]<=command_i[127:104];slot_q<=0;status_q<=OK;
   ...
   S_ROOT_RSP:if(descriptor_rsp_valid_i&&descriptor_rsp_ready_o)begin
    ...
    else if(descriptor_rsp_data_i[7:0]!=8'h01||descriptor_rsp_data_i[107:104]!=0||descriptor_rsp_data_i[111:108]!=5||descriptor_rsp_data_i[115:112]!=0||descriptor_rsp_data_i[55:32]==24'hffffff)begin status_q<=UNSUPPORTED;state_q<=S_CONTEXT;end
    else begin addr_q[slot_q]<={descriptor_rsp_data_i[127:120],descriptor_rsp_data_i[103:56]};shape_index_q<=descriptor_rsp_data_i[55:32];state_q<=S_SHAPE_REQ;end end
