...
module qwen2_shared_l2_matrix_tile16_payload #(parameter integer ADDR_W=15)(
 ...
 typedef enum logic[3:0]{I,ARQ,ARP,WRQ,WRP,MQ,MWAIT,OW,D}st_e;st_e st;logic[15:0]k_q,depth_q;logic[63:0]activation_q,weight_q,output_q;logic[3:0]row_q;logic[511:0]a_q,w_q;logic[16383:0]acc_q;logic final_seen_q;integer c;
 function automatic[15:0]bf16(input logic[31:0]v);logic[31:0]r;begin r=v+32'h7fff+v[16];return r[31:16];end endfunction
 assign l2_rd_valid_o=st==ARQ||st==WRQ;assign l2_rsp_ready_o=st==ARP||st==WRP;assign l2_rd_addr_o=st==ARQ?ADDR_W'(activation_q[ADDR_W+5:6]+k_q):ADDR_W'(weight_q[ADDR_W+5:6]+k_q);assign matrix_step_valid_o=st==MQ;assign matrix_context_o=0;assign matrix_clear_o=k_q==0;assign matrix_last_o=k_q==depth_q-1;assign matrix_a_o=a_q[255:0];assign matrix_b_o=w_q;assign matrix_out_ready_o=1;assign l2_wr_valid_o=st==OW;assign l2_wr_addr_o=ADDR_W'(output_q[ADDR_W+5:6]+row_q);assign l2_wr_be_o='1;assign done_o=st==D;
 always_comb begin l2_wr_data_o=0;for(c=0;c<32;c++)l2_wr_data_o[c*16+:16]=bf16(acc_q[(row_q*32+c)*32+:32]);end
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin st<=I;k_q<=0;depth_q<=0;activation_q<=0;weight_q<=0;output_q<=0;row_q<=0;a_q<=0;w_q<=0;acc_q<=0;final_seen_q<=0;read_beats_o<=0;write_beats_o<=0;matrix_steps_o<=0;end else begin if(matrix_out_valid_i&&matrix_out_ready_o&&matrix_out_last_i)begin acc_q<=matrix_acc_i;final_seen_q<=1;end case(st)
  I:if(start_i&&depth_i!=0)begin k_q<=0;depth_q<=depth_i;activation_q<=activation_local_i;weight_q<=weight_local_i;output_q<=output_local_i;row_q<=0;final_seen_q<=0;read_beats_o<=0;write_beats_o<=0;matrix_steps_o<=0;st<=ARQ;end
