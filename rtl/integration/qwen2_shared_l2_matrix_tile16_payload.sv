...
module qwen2_shared_l2_matrix_tile16_payload #(parameter integer ADDR_W=15)(
 ...
 typedef enum logic[3:0]{I,ARQ,ARP,WRQ,WRP,MQ,MWAIT,OW,D}st_e;st_e st;logic[15:0]k_q,depth_q,rows_q,columns_q;logic[63:0]activation_q,weight_q,output_q;logic[3:0]row_q;logic[511:0]a_q,w_q;logic[16383:0]acc_q;logic final_seen_q;integer c;
 ...
 assign legal=depth_i!=0&&rows_i>0&&rows_i<=16&&columns_i>0&&columns_i<=32&&
   ...
   {1'b0,weight_local_i}+({49'd0,depth_i}<<6)<=CAPACITY_BYTES&&
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin st<=I;k_q<=0;depth_q<=0;rows_q<=0;columns_q<=0;status_o<=0;activation_q<=0;weight_q<=0;output_q<=0;row_q<=0;a_q<=0;w_q<=0;acc_q<=0;final_seen_q<=0;read_beats_o<=0;write_beats_o<=0;matrix_steps_o<=0;end else begin if(matrix_out_valid_i&&matrix_out_ready_o&&matrix_out_last_i)begin acc_q<=matrix_acc_i;final_seen_q<=1;end case(st)
  I:if(start_i)begin k_q<=0;depth_q<=depth_i;rows_q<=rows_i;columns_q<=columns_i;activation_q<=activation_local_i;weight_q<=weight_local_i;output_q<=output_local_i;row_q<=0;final_seen_q<=0;read_beats_o<=0;write_beats_o<=0;matrix_steps_o<=0;status_o<=legal?0:5;st<=legal?ARQ:D;end
