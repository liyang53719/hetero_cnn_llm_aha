...
module qwen2_shared_l2_rms_tile16_stager #(parameter integer ADDR_W=15)(
 input logic clk_i,input logic rst_ni,input logic start_i,input logic transpose_only_i,input logic[63:0]hidden_local_i,weight_local_i,norm_token_local_i,norm_kmajor_local_i,
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin st<=I;beat_q<=0;token_q<=0;k_q<=0;x_q<=0;weight_q<=0;transpose_q<=0;flags_q<=0;read_beats_o<=0;write_beats_o<=0;rms_vectors_o<=0;transpose_values_o<=0;end else case(st)
  I:if(start_i)begin beat_q<=0;token_q<=0;k_q<=0;transpose_q<=0;flags_q<=0;read_beats_o<=0;write_beats_o<=0;rms_vectors_o<=0;transpose_values_o<=0;st<=((transpose_only_i===1'b1)?TQ:WQ);end
