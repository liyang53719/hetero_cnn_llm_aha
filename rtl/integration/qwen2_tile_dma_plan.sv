...
module qwen2_tile_dma_plan #(
  ...
  input logic[5:0]q_column_tile_i,input logic start_store_i,
  ...
  state_e state_q;logic[55:0]addr_q[0:5];logic[5:0]tile_q;integer i;
  ...
  always_ff@(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)begin state_q<=S_IDLE;status_o<=ST_OK;ddr_read_bytes_o<=0;ddr_write_bytes_o<=0;tile_q<=0;
      for(i=0;i<6;i++)addr_q[i]<=0;
    end else case(state_q)
      S_IDLE:if(context_valid_i&&context_ready_o)begin
        for(i=0;i<6;i++)addr_q[i]<=tensor_address_i[i*56+:56];tile_q<=q_column_tile_i;
