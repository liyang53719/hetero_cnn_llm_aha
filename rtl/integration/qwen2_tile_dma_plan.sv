...
module qwen2_tile_dma_plan #(
  ...
  input logic[5:0]q_column_tile_i,input logic[31:0]weight_src_stride_i,input logic[31:0]token_index_i,output_token_bytes_i,input logic full_q_i,input logic reuse_norm_i,input logic start_store_i,
  ...
  state_e state_q;logic[55:0]addr_q[0:5];logic[5:0]tile_q;logic[31:0]weight_stride_q,token_index_q,output_token_bytes_q;logic full_q_q,reuse_norm_q;integer i;
  ...
  always_comb begin
    ...
    case(state_q)
      S_L0_REQ:begin dma_src_addr_o={8'd0,addr_q[0]}+64'(token_index_q)*3072;dma_dst_addr_o=hidden_local_o;dma_row_bytes_o=3072;dma_src_stride_o=3072;dma_dst_stride_o=3072;end
      ...
      S_L2_REQ:begin dma_req_kind_o=DMA_LOAD_2D;dma_src_addr_o={8'd0,addr_q[4]}+64'(tile_q)*64;dma_dst_addr_o=q_weight_local_o;dma_row_bytes_o=64;dma_rows_o=1536;dma_src_stride_o=weight_stride_q;dma_dst_stride_o=64;end
      S_STORE_REQ:begin dma_req_kind_o=DMA_STORE_1D;dma_src_addr_o=q_output_local_o;dma_dst_addr_o={8'd0,addr_q[5]}+64'(token_index_q)*output_token_bytes_q+64'(tile_q)*64;dma_row_bytes_o=64;dma_src_stride_o=64;dma_dst_stride_o=64;end
      default:begin end
    endcase
  end
  always_ff@(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)begin state_q<=S_IDLE;status_o<=ST_OK;ddr_read_bytes_o<=0;ddr_write_bytes_o<=0;tile_q<=0;weight_stride_q<=0;token_index_q<=0;output_token_bytes_q<=0;full_q_q<=0;reuse_norm_q<=0;
      for(i=0;i<6;i++)addr_q[i]<=0;
    end else case(state_q)
      S_IDLE:if(context_valid_i&&context_ready_o)begin
        for(i=0;i<6;i++)addr_q[i]<=tensor_address_i[i*56+:56];tile_q<=q_column_tile_i;weight_stride_q<=weight_src_stride_i;token_index_q<=token_index_i;output_token_bytes_q<=output_token_bytes_i;full_q_q<=full_q_i;reuse_norm_q<=reuse_norm_i;
