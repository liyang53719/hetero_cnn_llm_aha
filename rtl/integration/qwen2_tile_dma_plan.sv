...
module qwen2_tile_dma_plan #(
  ...
  state_e state_q;logic[55:0]addr_q[0:5];integer i;
  ...
  assign norm_local_o=LOCAL_BASE+64'h3000;assign q_weight_local_o=LOCAL_BASE+64'h4000;
  assign q_output_local_o=LOCAL_BASE+64'h1c000;
  always_comb begin
    ...
    case(state_q)
      ...
      S_L2_REQ:begin dma_req_kind_o=DMA_LOAD_2D;dma_src_addr_o={8'd0,addr_q[4]};dma_dst_addr_o=q_weight_local_o;dma_row_bytes_o=64;dma_rows_o=1536;dma_src_stride_o=3072;dma_dst_stride_o=64;end
      S_STORE_REQ:begin dma_req_kind_o=DMA_STORE_1D;dma_src_addr_o=q_output_local_o;dma_dst_addr_o={8'd0,addr_q[5]};dma_row_bytes_o=64;dma_src_stride_o=64;dma_dst_stride_o=64;end
      default:begin end
    endcase
  end
  always_ff@(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)begin state_q<=S_IDLE;status_o<=ST_OK;ddr_read_bytes_o<=0;ddr_write_bytes_o<=0;
      for(i=0;i<6;i++)addr_q[i]<=0;
    end else case(state_q)
      S_IDLE:if(context_valid_i&&context_ready_o)begin
        for(i=0;i<6;i++)addr_q[i]<=tensor_address_i[i*56+:56];
        status_o<=context_legal_i?ST_OK:ST_UNSUPPORTED;ddr_read_bytes_o<=0;ddr_write_bytes_o<=0;
        state_q<=context_legal_i?S_L0_REQ:S_DONE;
      end
