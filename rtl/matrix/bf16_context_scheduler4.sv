...
// Fixed four-context command/completion scheduler; no payload storage.
`timescale 1ns/1ps
module bf16_context_scheduler4 (
  input  logic clk_i,
  input  logic rst_ni,
  ...
  output logic last_o,
  output logic array_in_valid_o,
  input  logic array_in_ready_i,
  input  logic array_out_valid_i,
  output logic array_out_ready_o,
  output logic [1:0] issue_context_o,
  output logic issue_clear_o,
  output logic issue_bypass_o,
  output logic issue_use_bank_o,
  output logic completion_fire_o,
  output logic [1:0] completion_context_o,
  ...
  logic [3:0] busy_q, accumulator_valid_q;
  logic [1:0] context_fifo [0:7];
  logic last_fifo [0:7];
  logic [2:0] write_pointer_q, read_pointer_q;
  logic [3:0] fifo_count_q;
  logic fifo_not_full, fifo_not_empty, completion_same_context;
  logic context_available, issue_fire;
  ...
  assign context_available = !busy_q[context_i] || completion_same_context;
  ...
  assign issue_bypass_o = completion_same_context;
  assign issue_use_bank_o = accumulator_valid_q[context_i];
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (array_out_valid_i && !fifo_not_empty)
      ...
      if (issue_fire) begin
        context_fifo[write_pointer_q] <= context_i;
        last_fifo[write_pointer_q] <= last_i;
        write_pointer_q <= write_pointer_q + 1'b1;
        busy_q[context_i] <= 1'b1;
