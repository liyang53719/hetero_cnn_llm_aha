...
module bf16_context_front_control5_rev8b_b_candidate (
  ...
  logic scheduler_error,tag_mismatch_q;
  ...
  always_ff @(posedge clk_i or negedge rst_ni)begin
    if(!rst_ni)tag_mismatch_q<=0;
    else if(out_valid_o&&output_context_o!=context_o)tag_mismatch_q<=1'b1;
  end
  assign protocol_error_o=scheduler_error|tag_mismatch_q;
endmodule
