...
// Synthesizable protocol wrapper for ARM CLN22UL l2sp6144x128wm.
`timescale 1ns/1ps
module l2_sp6144x128_macro_wrapper (
  input  logic         clk_i,
  input  logic         rst_ni,
  ...
  localparam logic [12:0] WORDS = 13'd6144;
  typedef enum logic [2:0] {S_IDLE, S_ACCESS, S_CAPTURE, S_RESP} state_e;
  state_e state_q;
  logic write_q, error_q;
  logic [12:0] addr_q;
  logic [127:0] wdata_q, rdata_q, macro_q;
  logic [15:0] wstrb_q;
  logic [127:0] macro_wen_n;
  logic macro_cen_n;
  integer byte_index;
  ...
  l2sp6144x128wm u_macro (
    .q(macro_q),
    .clk(clk_i),
    .cen(macro_cen_n),
    .gwen(write_q ? 1'b0 : 1'b1),
    .a(addr_q),
    .d(wdata_q),
    .wen(macro_wen_n),
    .stov(1'b0),
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      case (state_q)
        ...
        S_ACCESS: begin
          state_q <= write_q ? S_RESP : S_CAPTURE;
        end
        S_CAPTURE: begin
          rdata_q <= macro_q;
          state_q <= S_RESP;
