...
`ifndef SYNTHESIS
 initial assert((LANES==1)||(LANES==2));always_ff@(posedge clk_i)if(rst_ni&&(|lane_out_valid))begin assert(&lane_out_valid);for(integer i=1;i<LANES;i=i+1)begin assert(lane_tag[i*TAG_W+:TAG_W]==lane_tag[0+:TAG_W]);assert(lane_last[i]==lane_last[0]);end end
`endif
endmodule
