...
`ifndef SYNTHESIS
 always_ff@(posedge clk_i or negedge rst_ni)if(!rst_ni)begin end else begin if((|mav)&&!(&mav))$fatal(1,"mulA desync");if((|mbv)&&!(&mbv))$fatal(1,"mulB desync");if((|av)&&!(&av))$fatal(1,"add desync");if(pair_fire&&mal!==mbl)$fatal(1,"last mismatch");if(out_valid_o&&al!=={LANES{al[0]}})$fatal(1,"last mismatch");end
`endif
endmodule
