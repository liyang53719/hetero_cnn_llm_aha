...
module fp32_norm16_rms_l2_pipe(
 ...
 input logic[511:0]x_i,weight_i,input logic[31:0]epsilon_i,output logic out_valid_o,input logic out_ready_i,
 ...
 logic[3:0]st;logic mode_q;logic[1:0]vphase;logic scalar_add;logic[511:0]xq,wq,tmpq,yq;logic[31:0]epsq,scalar_q,norm_q,invq;
 ...
 HeteroFP32MulPipeBit1 sm(.clock(clk_i),.reset(!rst_ni),.io_inValid(siv),.io_inReady(sir),.io_x(scalar_q),.io_y(32'h3d800000),.io_userIn(0),.io_outValid(sov),.io_outReady(sor),.io_out(so),.io_exceptionFlags(sf),.io_userOut(su));
 ...
 always_ff@(posedge clk_i or negedge rst_ni)begin if(!rst_ni)begin st<=IDLE;mode_q<=0;vphase<=0;xq<=0;wq<=0;tmpq<=0;yq<=0;epsq<=0;scalar_q<=0;norm_q<=0;invq<=0;flags_q<=0;scalar_add<=0;end else case(st)
  IDLE:if(in_valid_i)begin mode_q<=mode_rms_i;xq<=x_i;wq<=weight_i;epsq<=epsilon_i;flags_q<=0;vphase<=0;st<=VISSUE;end
