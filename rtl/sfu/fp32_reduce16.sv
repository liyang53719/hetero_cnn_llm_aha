...
module fp32_reduce16(
  ...
  genvar i;
  generate for(i=0;i<8;i++)begin:g_l1 HeteroFP32Alu u(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),
    .io_x(input_q[(2*i)*32 +:32]),.io_y(input_q[(2*i+1)*32 +:32]),.io_out(l1[i*32 +:32]),.io_exceptionFlags(flags[i*5 +:5]));end
    for(i=0;i<4;i++)begin:g_l2 HeteroFP32Alu u(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),
    .io_x(l1[(2*i)*32 +:32]),.io_y(l1[(2*i+1)*32 +:32]),.io_out(l2[i*32 +:32]),.io_exceptionFlags(flags[(8+i)*5 +:5]));end
    for(i=0;i<2;i++)begin:g_l3 HeteroFP32Alu u(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),
    .io_x(l2[(2*i)*32 +:32]),.io_y(l2[(2*i+1)*32 +:32]),.io_out(l3[i*32 +:32]),.io_exceptionFlags(flags[(12+i)*5 +:5]));end
  endgenerate
  HeteroFP32Alu u_l4(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),.io_x(l3[31:0]),.io_y(l3[63:32]),
