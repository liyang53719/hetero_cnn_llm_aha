...
module fp32_rope_pair(
 ...
 HeteroFP32Alu m0(.clock(clk_i),.reset(!rst_ni),.io_op(1'b1),.io_x(e_q),.io_y(c_q),.io_out(ec),.io_exceptionFlags(flags[0+:5]));
 HeteroFP32Alu m1(.clock(clk_i),.reset(!rst_ni),.io_op(1'b1),.io_x(o_q),.io_y(s_q),.io_out(os),.io_exceptionFlags(flags[5+:5]));
 HeteroFP32Alu m2(.clock(clk_i),.reset(!rst_ni),.io_op(1'b1),.io_x(e_q),.io_y(s_q),.io_out(es),.io_exceptionFlags(flags[10+:5]));
 HeteroFP32Alu m3(.clock(clk_i),.reset(!rst_ni),.io_op(1'b1),.io_x(o_q),.io_y(c_q),.io_out(oc),.io_exceptionFlags(flags[15+:5]));
 HeteroFP32Alu a0(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),.io_x(ec),.io_y({~os[31],os[30:0]}),.io_out(er),.io_exceptionFlags(flags[20+:5]));
 HeteroFP32Alu a1(.clock(clk_i),.reset(!rst_ni),.io_op(1'b0),.io_x(es),.io_y(oc),.io_out(orr),.io_exceptionFlags(flags[25+:5]));
