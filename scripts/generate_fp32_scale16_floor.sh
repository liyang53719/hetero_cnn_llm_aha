...
test -z "$(git -C "$CHIP" status --porcelain)"||exit 3;CMD=";project chipyard;set Global / concurrentRestrictions := Seq(Tags.limitAll(8));set Compile / unmanagedSources += file(\"$SRC\");runMain gemmini.EmitHeteroFP32Scale16Floor $OUT/HeteroFP32Scale16Floor.sv"
