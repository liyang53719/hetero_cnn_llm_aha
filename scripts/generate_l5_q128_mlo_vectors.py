...
GQ=GK=GV=None;SEGMENTS=256;SOFTMAX_BLOCK=0
...
def compute_qt(qt):
 ...
 for h in range(HEADS):
  sl=slice(h*D,(h+1)*D);m=l=None;o=None;tm=-math.inf;tl=0.;to=np.zeros(D,np.float64);block=SOFTMAX_BLOCK or qt+1
  for bs in range(0,qt+1,block):
   bm=bl=None;bo=None
   for kt in range(bs,min(qt+1,bs+block)):
    s=dot(q[qt,sl],k[kt,sl]);sf=float(s)
    if kt==bs:bm=s;bl=np.float32(1);bo=v[kt,sl].copy()
    else:
     mn=s if s>bm else bm;alpha=exp2p(mul(add(bm,-mn),log2e));beta=exp2p(mul(add(s,-mn),log2e));bl=add(mul(bl,alpha),beta);bo=np.array([add(mul(bo[i],alpha),mul(v[kt,sl][i],beta))for i in range(D)],np.float32);bm=mn
   ...
   if m is None:m=bm;l=bl;o=bo
   else:
    mn=bm if bm>m else m;alpha=exp2p(mul(add(m,-mn),log2e));beta=exp2p(mul(add(bm,-mn),log2e));l=add(mul(l,alpha),mul(bl,beta));o=np.array([add(mul(o[i],alpha),mul(bo[i],beta))for i in range(D)],np.float32);m=mn
...
def main():
 ...
 updates=a.tokens*(a.tokens+1)//2*12;merges=sum(max(0,math.ceil((qt+1)/(SOFTMAX_BLOCK or qt+1))-1)for qt in range(a.tokens))*12;man={'input_sha256':hashes,'tokens':a.tokens,'causal_updates':updates,'softmax_block':SOFTMAX_BLOCK,'summary_merges':merges,'reciprocals':a.tokens*12,'normalization_chunks':a.tokens*96,'score_matrix_materialized':False,'max_attention_error':maxerr,'threshold':0.002,'pass':maxerr<=.002,'node_sha256':{n:hashlib.sha256((a.out/f'{n}.memh').read_bytes()).hexdigest()for n in nodes}}
