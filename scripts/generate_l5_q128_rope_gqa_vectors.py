...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--qkv-dir',type=Path,required=True);ap.add_argument('--tokens',type=int,choices=(128,384),default=128);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args();a.out.mkdir(parents=True,exist_ok=True);hashes=ALL_HASHES[a.tokens]
