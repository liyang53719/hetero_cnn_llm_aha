...
with safe_open(MODEL,framework='pt',device='cpu')as f:
 emb=f.get_tensor('model.embed_tokens.weight')[torch.tensor(ids[:16],dtype=torch.long)].float().numpy();nw=f.get_tensor('model.layers.0.input_layernorm.weight').float().numpy();qw=f.get_tensor('model.layers.0.self_attn.q_proj.weight')[:32].contiguous().view(torch.uint16).numpy()
...
pack(OUT/'activation_kmajor.memh',a);pack(OUT/'q_weight_tile0.memh',weights);pack(OUT/'expected_rows.memh',expected)
