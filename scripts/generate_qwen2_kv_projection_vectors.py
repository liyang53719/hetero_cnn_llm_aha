...
with safe_open(ROOT/'work/models/qwen2_1p5b_instruct_ba1cf184/model.safetensors',framework='pt',device='cpu') as f:
 for kind in ('k','v'):
  ...
  raw=torch.nn.functional.linear(norm,w,None).contiguous();biased=torch.nn.functional.linear(norm,w,b).contiguous();rawb=raw.view(torch.uint16);biasedb=biased.view(torch.uint16)
  (OUT/f'{kind}_weight_all_tiles.memh').write_text(pack([wb[t*32+c,k]for t in range(8)for k in range(1536)for c in range(32)]));(OUT/f'{kind}_raw_expected_beats.memh').write_text(pack(rawb));(OUT/f'{kind}_bias_beats.memh').write_text(pack(bb));(OUT/f'{kind}_biased_expected_beats.memh').write_text(pack(biasedb));
