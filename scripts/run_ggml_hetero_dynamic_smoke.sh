#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.."&&pwd);UP=$ROOT/work/upstream/llama_cpp;BUILD=$ROOT/work/results/llama_cpp_pinned_build/bin;INPUT=$ROOT/work/results/qwen2_q1024_full28_inputs;OUT=${OUT:-$ROOT/work/results/ggml_hetero_dynamic_smoke};RUN=$ROOT/scripts/run_memory_capped.sh;mkdir -p "$INPUT" "$OUT"
...
run 600s /usr/bin/g++ -std=c++20 -O2 -I"$UP/ggml/include" -I"$UP/ggml/src" "$ROOT/cpp/ggml_hetero_dynamic_smoke.cpp" -L"$BUILD" -Wl,-rpath,"$BUILD" -lggml -lggml-base -lggml-cpu -pthread -ldl -o "$OUT/smoke"
run 600s env OMP_NUM_THREADS=8 OMP_PROC_BIND=true "$OUT/smoke" "$OUT/libggml-hetero.so" "$INPUT" "$OUT/payload"|tee "$OUT/run.log"
