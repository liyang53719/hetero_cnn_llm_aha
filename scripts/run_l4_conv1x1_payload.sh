...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$RUN" "$TOOLS/bin/riscv64-unknown-elf-gcc" -B"$TMP/" \
...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$RUN" "$SIM" +permissive +dramsim \
...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$RUN" "$PY" "$ROOT/scripts/audit_l4_conv1x1_payload.py" \
