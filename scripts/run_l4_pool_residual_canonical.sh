...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$V" --lint-only --timing -Wall --top-module tb_hetero_l3_stream_complex "${S[@]}" >"$OUT/lint.log" 2>&1
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$V" --binary --timing -Wall -j 8 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" -GTARGET=0 -GDEDICATED_TARGET=10000 --top-module tb_hetero_l3_stream_complex --Mdir "$OUT/obj_10k" -o tb "${S[@]}" >"$OUT/build.log" 2>&1
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$OUT/obj_10k/tb"|tee "$OUT/tb.log"
