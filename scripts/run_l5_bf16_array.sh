...
run_case(){ local rows=$1 cols=$2 tag=$3;taskset -c 8-23 "$PY" "$ROOT/scripts/generate_bf16_array_expected.py" --rows "$rows" --cols "$cols" --steps 4 --output "$OUT/expected.memh"
 ...
 MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$V" --binary --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL -j 8 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" -GROWS="$rows" -GCOLS="$cols" --top-module tb_bf16_outer_product_array --Mdir "$OUT/obj_$tag" -o tb "$LANE" "$ROOT/rtl/matrix/bf16_outer_product_array.sv" "$ROOT/tb/tb_bf16_outer_product_array.sv" >"$OUT/build_$tag.log" 2>&1
 MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$OUT/obj_$tag/tb"|tee "$OUT/tb_$tag.log";}
run_case 2 2 2x2
