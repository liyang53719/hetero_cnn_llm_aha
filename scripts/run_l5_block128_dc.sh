...
OUT="$ROOT/work/results/l5_block128_merge/dc"
mkdir -p "$OUT"

TOP=fp32_mlo_summary_merge_stream \
RTL_FILELIST="$ROOT/dc/filelists/l5_block128_merge.f" \
STD_CELL_DBS="$DB" CLOCK_PERIOD_NS=1.0 CLOCK_PORT=clk_i \
OUT_DIR="$OUT" DC_MAX_CORES=8 \
...
if wns < 0:
    raise SystemExit(f"L5_BLOCK128_DC_TIMING_FAIL WNS={wns}")
print(f"L5_BLOCK128_DC_PASS WNS_NS={wns} UNMAPPED=0")
PY
