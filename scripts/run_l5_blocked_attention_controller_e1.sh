...
MIN_AVAILABLE_KIB=4194304 MEMORY_HIGH=12G MEMORY_MAX=16G "$R" "$V" --binary --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL -Wno-WIDTHEXPAND -Wno-WIDTHTRUNC -j 8 --top-module tb_blocked_attention_stream_controller --Mdir "$OUT/obj" -o tb "${SOURCES[@]}" >"$OUT/build.log" 2>&1
MIN_AVAILABLE_KIB=4194304 MEMORY_HIGH=12G MEMORY_MAX=16G "$R" "$OUT/obj/tb"|tee "$OUT/tb.log"
