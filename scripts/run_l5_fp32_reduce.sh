...
"$ROOT/scripts/generate_fp32_alu.sh";A=$ROOT/work/results/l5_fp32_alu;mkdir -p "$A"
...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=8G MEMORY_MAX=10G "$R" "$V" --lint-only --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL --top-module tb_fp32_reduce16 "$ROOT/work/generated/l5_fp32_alu/HeteroFP32Alu.sv" "$ROOT/rtl/sfu/fp32_reduce16.sv" "$ROOT/tb/tb_fp32_reduce16.sv" >"$B/lint.log" 2>&1
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=8G MEMORY_MAX=10G "$R" "$V" --binary --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL -j 4 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" --top-module tb_fp32_reduce16 --Mdir "$B/obj" -o tb "$ROOT/work/generated/l5_fp32_alu/HeteroFP32Alu.sv" "$ROOT/rtl/sfu/fp32_reduce16.sv" "$ROOT/tb/tb_fp32_reduce16.sv" >"$B/build.log" 2>&1
