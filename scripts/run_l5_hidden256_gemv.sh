...
"$ROOT/scripts/generate_all_hardfloat_primitives.sh";taskset -c 8-23 "$PY" "$ROOT/scripts/generate_l5_hidden256_gemv_vectors.py" --out "$OUT/vectors"
...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$V" --binary --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL -j 8 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" --top-module tb_l5_hidden256_gemv --Mdir "$OUT/obj" -o tb "$ROOT/work/generated/l5_all_primitives/HeteroAllPrimitives.sv" "$ROOT/rtl/matrix/bf16_outer_product_array.sv" "$ROOT/tb/tb_l5_hidden256_gemv.sv" >"$OUT/build.log" 2>&1
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$OUT/obj/tb"|tee "$OUT/tb.log";grep -q 'L5_HIDDEN256_GEMV_PASS steps=2048' "$OUT/tb.log";echo L5_HIDDEN256_GEMV_GATE_PASS
