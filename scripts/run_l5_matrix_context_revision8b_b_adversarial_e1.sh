#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.." && pwd);OUT=${OUT:-$ROOT/work/results/l5_matrix_context_revision8b_b/adversarial_e1}
...
SOURCES=("$ROOT/work/generated/l5_all_primitives/HeteroAllPrimitives.sv" "$ROOT/rtl/matrix/bf16_outer_product_array_glue512.sv"
 ...
 "$ROOT/tb/tb_bf16_outer_product_context_array_rev8b_b_adversarial.sv")
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" timeout --foreground --signal=INT --kill-after=30s 600s "$V" \
 ...
 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" --top-module tb_bf16_outer_product_context_array_rev8b_b_adversarial \
...
grep -q 'BF16_CONTEXT5_ADVERSARIAL_PASS steps=50000' "$OUT/tb.log"
