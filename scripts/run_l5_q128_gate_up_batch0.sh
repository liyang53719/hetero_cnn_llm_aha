...
taskset -c 8-23 "$PY" "$ROOT/scripts/manifest_l5_q128_gate_up_batch.py" --batch 0 --norm2 "$ROOT/work/results/l5_q128_oproj/batch0/norm2.memh" --gate "$OUT/gate.memh" --up "$OUT/up.memh" --output "$OUT/manifest.json"
...
cd "$ROOT";for m in 0 1;do MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$OUT/obj/tb" +BATCH=0 +MODE="$m"|tee "$OUT/mode$m.log";grep -q "L5_Q128_GATE_UP_BATCH_PASS batch=0 mode=$m" "$OUT/mode$m.log";done;echo L5_Q128_GATE_UP_BATCH0_GATE_PASS
