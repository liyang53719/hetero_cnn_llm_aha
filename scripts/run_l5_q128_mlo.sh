...
taskset -c 8-23 "$PY" "$ROOT/scripts/generate_l5_q128_mlo_vectors.py" --rope-dir "$ROOT/work/results/l5_q128_rope_gqa/vectors" --tokens 128 --out "$OUT/vectors"
...
cd "$ROOT";MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" "$OUT/obj/tb" +WORKLOAD=128|tee "$OUT/tb.log";grep -q 'L5_Q_PREFILL_MLO_PASS workload=128 updates=99072 reciprocals=1536' "$OUT/tb.log";echo L5_Q128_MLO_GATE_PASS
