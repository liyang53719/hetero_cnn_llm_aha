...
"$ROOT/scripts/generate_fp32_primitives.sh";taskset -c 8-25 "$ROOT/work/toolchain/cnn_py312/bin/python" "$ROOT/scripts/generate_fp32_rope_vectors.py" --output "$OUT/vectors.memh"
...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=8G MEMORY_MAX=10G "$R" "$V" --binary --timing -Wall -Wno-DECLFILENAME -Wno-TIMESCALEMOD -Wno-UNUSEDSIGNAL -j 4 -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" --top-module tb_fp32_rope_pair --Mdir "$OUT/obj" -o tb "$ROOT/work/generated/l5_fp32_primitives/HeteroFP32Primitives.sv" "$ROOT/rtl/sfu/fp32_rope_pair.sv" "$ROOT/tb/tb_fp32_rope_pair.sv" >"$OUT/build.log" 2>&1
