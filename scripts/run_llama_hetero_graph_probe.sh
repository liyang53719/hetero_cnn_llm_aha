...
run 600s /usr/bin/g++ -std=c++20 -O2 -I"$UP/include" -I"$UP/ggml/include" "$ROOT/cpp/llama_hetero_graph_probe.cpp" -L"$BUILD" -Wl,-rpath,"$BUILD" -lllama -lggml -lggml-base -lggml-cpu -pthread -ldl -o "$OUT/probe"
run 600s "$OUT/probe" "$OUT/libggml-hetero.so" "$GGUF" "$INPUT" "$OUT/payload" "$OUT/partition.tsv"|tee "$OUT/run.log"
