...
MIN_AVAILABLE_KIB=10485760 MEMORY_HIGH=24G MEMORY_MAX=30G "$R" timeout --foreground --signal=INT --kill-after=30s 600s \
 ...
 "$ROOT/work/generated/l5_all_primitives/HeteroAllPrimitives.sv" "$ROOT/rtl/sfu/fp32_reduce16.sv" "$ROOT/rtl/sfu/fp32_reduce_max16.sv" \
 "$ROOT/rtl/integration/operator_sfu_vector_endpoint_v3.sv" "$ROOT/tb/tb_operator_sfu_vector_endpoint_v3.sv" >"$OUT/build.log" 2>&1
...
grep -q 'OPERATOR_SFU_VECTOR_ENDPOINT_V3_PASS opcodes=12' "$OUT/test.log"
