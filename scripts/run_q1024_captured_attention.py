...
def main():
    out=ROOT/'work/results/q1024_captured_attention';out.mkdir(exist_ok=True)
    ...
    identity={str(p.relative_to(ROOT)):sha(p) for p in [sim,vectors/'manifest.json',*sorted(vectors.glob('*.memh'))]}
    receipts=[]
    for qt in range(64):
        ...
        if receipt.exists():
            ...
            assert sha(Path(old['log']))==old['log_sha256'];receipts.append(old);continue
        log=out/f'qt{qt:02d}.log'
        cmd=['bash','scripts/run_memory_capped.sh','timeout','--signal=INT','--kill-after=30s','600',
             str(sim),f'+VECTORS={vectors}','+FULL_Q1024',f'+QT={qt}']
    ...
    report=dict(status='PASS_Q1024_ALL_ROW_ATTENTION_NUMERICAL_PARTITIONS',identity=identity,
                ...
                partitions=[dict(qt=r['qt'],log_sha256=r['log_sha256'],output_hash=r['output_hash']) for r in receipts],
