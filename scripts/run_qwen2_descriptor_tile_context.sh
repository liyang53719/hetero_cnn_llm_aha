#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.."&&pwd);OUT=${OUT:-$ROOT/work/results/qwen2_descriptor_tile_context}
...
run 600s "$V" --binary --timing --assert -Wall -Wno-fatal -Wno-DECLFILENAME -Wno-TIMESCALEMOD \
  -Wno-UNUSEDSIGNAL -Wno-WIDTH -Wno-SYNCASYNCNET -Wno-PROCASSINIT -j 8 \
  -MAKEFLAGS "AR=/usr/bin/ar CXX=/usr/bin/g++" --top-module tb_qwen2_descriptor_tile_context \
  --Mdir "$OUT/obj" -o tb "$ROOT/rtl/fabric/shared_l2_fabric.sv" \
  "$ROOT/rtl/fabric/shared_l2_descriptor_port.sv" "$ROOT/rtl/integration/qwen2_descriptor_tile_context.sv" \
  "$ROOT/tb/tb_qwen2_descriptor_tile_context.sv" >"$OUT/build.log" 2>&1
cd "$ROOT";run 600s "$OUT/obj/tb"|tee "$OUT/tb.log"
grep -q 'QWEN2_DESCRIPTOR_TILE_CONTEXT_PASS roots=6 descriptor_fetches=12' "$OUT/tb.log"
