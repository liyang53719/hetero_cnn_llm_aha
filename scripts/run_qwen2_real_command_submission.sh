#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.."&&pwd);OUT=${OUT:-$ROOT/work/results/qwen2_real_command_submission};SRC=$ROOT/reports/execution/llama_cpp_qwen2_graph_lowering_manifest.jsonl;R=$ROOT/scripts/run_memory_capped.sh;V=${VERILATOR:-$ROOT/work/toolchain/conda/bin/verilator};MACRO=$ROOT/work/generated/l10_sram/ct_sp_4096x128wm_base_0p8v_tt25/ctsp4096x128wm.v;mkdir -p "$OUT" "$ROOT/work/results/llama_cpp_qwen2_baseline"
taskset -c 8-23 python3 - "$SRC" "$ROOT/work/results/llama_cpp_qwen2_baseline/commands.memh" <<'PY'
import json,sys
records=[json.loads(line)for line in open(sys.argv[1])];open(sys.argv[2],'w').write(''.join(x['word'].removeprefix('0x')+'\n'for x in records))
PY
...
cd "$ROOT";run 600s "$OUT/obj/tb"|tee "$OUT/tb.log";grep -q 'QWEN2_REAL_COMMAND_SUBMISSION_PASS commands=588 completions=588 matrix=252 sfu=308 kv=28' "$OUT/tb.log"
