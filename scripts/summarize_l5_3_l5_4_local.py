...
q1024_path=ROOT/'reports/execution/l5_q1024_reviewed_e2_result.json';q1024=json.loads(q1024_path.read_text()) if q1024_path.is_file() else {}
stress_path=ROOT/'reports/execution/l5_attention_stress_service_result.json';stress=json.loads(stress_path.read_text()) if stress_path.is_file() else {}
...
checks={'controller_e1':all(x in text(controller_log) for x in ('seq=128 tasks=240 merges=0','seq=384 tasks=1872 merges=4608','seq=1024 tasks=12672 merges=43008','TB_PASS')),'controller_dc':controller_dc['wns_ns'] is not None and controller_dc['wns_ns']>=0 and controller_dc['unmapped']==0,'attention_trace_bridge':bridge.get('status')=='PASS_TRACE_COUPLED_BRIDGE','block32_weight_e1':'BLOCK32_SOFTMAX_WEIGHT_PASS rows=2 weights=64' in text(weight_log),'block32_weight_dc':weight_dc['wns_ns'] is not None and weight_dc['wns_ns']>=0 and weight_dc['unmapped']==0,'silu_1lane_e1':'TB_PASS LANES=1 cases=4096' in text(silu1_log),'silu_2lane_e1':'TB_PASS LANES=2 cases=4096' in text(silu2_log),'silu_1lane_dc':silu1_dc['wns_ns'] is not None and silu1_dc['wns_ns']>=0 and silu1_dc['unmapped']==0,'silu_2lane_dc':silu2_dc['wns_ns'] is not None and silu2_dc['wns_ns']>=0 and silu2_dc['unmapped']==0,'full_attention_numerical_e2':attempt.get('status')=='PASS_Q128_SINGLE_PROCESS_E2' and q384.get('status')=='PASS_Q384_SAMPLED_E2' and q1024.get('status')=='PASS_Q1024_REVIEWED_E2','silu_lane_selection':False}
...
r={'schema_version':1,'status':'L5_3_PASS_L5_4_SELECTION_OPEN' if components_pass and checks['full_attention_numerical_e2'] and stress.get('status')=='PASS' else 'FAIL_OR_INCOMPLETE','checks':checks,'L5.3':{'status':'PASS','controller_dc':controller_dc,'block32_weight_dc':weight_dc,'q128_tasks':240,'q384_tasks':1872,'q1024_tasks':12672,'q1024_merge_rows':43008,'trace_bridge_status':bridge.get('status'),'full_numeric_status':'PASS_Q128_Q384_Q1024','stress_service_evidence':'reports/execution/l5_attention_stress_service_result.json'},'L5.4':{'one_lane':silu1_dc,'two_lane':silu2_dc,'selection':'WAIT_MEASURED_MATRIX_PRODUCER_STALL','selection_rule':'one_lane_if_stall_le_2_percent_else_two_lane'},'q128_single_sim':{'evidence':'reports/execution/l5_q128_single_sim_attempt_result.json','status':attempt.get('status'),'tasks':attempt.get('observed_progress',{}).get('tasks'),'rows':attempt.get('observed_progress',{}).get('rows'),'rtl_cycles':attempt.get('observed_progress',{}).get('rtl_cycles')},'q384_sampled_e2':{'evidence':'reports/execution/l5_q384_sampled_e2_result.json','status':q384.get('status'),'compared_rows':q384.get('actual_compared_rows'),'controller_merge_rows':q384.get('controller_merge_rows'),'rtl_cycles':q384.get('rtl_cycles')},'q1024_reviewed_e2':{'evidence':'reports/execution/l5_q1024_reviewed_e2_result.json','status':q1024.get('status'),'compared_rows':q1024.get('actual_compared_rows'),'controller_merge_rows':q1024.get('controller_merge_rows'),'rtl_cycles':q1024.get('aggregate_rtl_cycles')},'stress_service':stress,'evidence_hashes':{'controller_e1':sha(controller_log),'trace_bridge':sha(bridge_path),'block32_weight_e1':sha(weight_log),'silu1_e1':sha(silu1_log),'silu2_e1':sha(silu2_log)}}
