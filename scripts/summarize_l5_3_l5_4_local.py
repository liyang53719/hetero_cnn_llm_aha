...
checks={'controller_e1':all(x in text(controller_log) for x in ('CASE_PASS seq=128 tasks=240 merges=0','CASE_PASS seq=384 tasks=1872 merges=4608','CASE_PASS seq=1024 tasks=12672 merges=43008','TB_PASS')),'controller_dc':controller_dc['wns_ns'] is not None and controller_dc['wns_ns']>=0 and controller_dc['unmapped']==0,'silu_1lane_e1':'TB_PASS LANES=1 cases=4096' in text(silu1_log),'silu_2lane_e1':'TB_PASS LANES=2 cases=4096' in text(silu2_log),'silu_1lane_dc':silu1_dc['wns_ns'] is not None and silu1_dc['wns_ns']>=0 and silu1_dc['unmapped']==0,'silu_2lane_dc':silu2_dc['wns_ns'] is not None and silu2_dc['wns_ns']>=0 and silu2_dc['unmapped']==0,'full_attention_numerical_e2':False,'silu_lane_selection':False}
r={'schema_version':1,'status':'SOURCE_LOCAL_GATES_PASS_INTEGRATION_OPEN' if all(checks[k] for k in ('controller_e1','controller_dc','silu_1lane_e1','silu_2lane_e1','silu_1lane_dc','silu_2lane_dc')) else 'FAIL_OR_INCOMPLETE','checks':checks,'L5.3':{'controller_dc':controller_dc,'q128_tasks':240,'q384_tasks':1872,'q1024_tasks':12672,'q1024_merge_rows':43008,'full_numeric_status':'OPEN'},'L5.4':{'one_lane':silu1_dc,'two_lane':silu2_dc,'selection':'WAIT_MEASURED_MATRIX_PRODUCER_STALL','selection_rule':'one_lane_if_stall_le_2_percent_else_two_lane'},'evidence_hashes':{'controller_e1':sha(controller_log),'silu1_e1':sha(silu1_log),'silu2_e1':sha(silu2_log)}}
out=ROOT/'reports/execution/l5_3_l5_4_local_result.json';out.write_text(json.dumps(r,indent=2,sort_keys=True)+'\n');print(json.dumps(r,indent=2,sort_keys=True));raise SystemExit(0 if r['status'].startswith('SOURCE_LOCAL_GATES_PASS') else 1)
