...
control=load('config/control_plane.json');policy=load('config/git_workflow_policy.json');ledger=load('reports/execution/MASTER_LEDGER.json');next_action=load('reports/execution/NEXT_ACTION.json');final=load('reports/final_validation.json');local=load('reports/execution/l5_3_l5_4_local_result.json');attempt=load('reports/execution/l5_q128_single_sim_attempt_result.json');q384=load('reports/execution/l5_q384_sampled_e2_result.json');q1024=load('reports/execution/l5_q1024_reviewed_e2_result.json');stress=load('reports/execution/l5_attention_stress_service_result.json');bridge=load('reports/execution/l5_attention_trace_bridge_result.json');probability=load('reports/execution/l5_probability_hilo_result.json');v69=load('reports/execution/sandbox_v69_result.json');l52=load('reports/execution/l5_revision8b_b_local_result.json')
...
assert control['current_subgate']=='L5.4_SILU_MEASURED_STALL_SELECTION'
assert control['current_state']=='L5_3_PASS_NEXT_L5_4_MEASURED_STALL_SELECTION'
assert l52['status']=='PASS' and l52['h3']['wns_ns']>=0
assert local['status']=='L5_3_PASS_L5_4_SELECTION_OPEN'
...
assert next_action['decision']=='MEASURE_SILU_PRODUCER_STALL_AND_SELECT_LANES'
assert next_action['L5.5_review']['review_floor_tps']==315
assert final['status']=='PASS_V6_10_L5_3_L5_4_SELECTION_OPEN'
...
result={'schema_version':6,'status':'PASS_V6_10_LOCAL_BRIDGE','branch_policy':'MAIN_ONLY','L5_2':'PASS','L5_3_controller':'PASS_E1_E4','L5_3_trace_bridge':'PASS_NOT_SINGLE_SIM','L5_3_block32_weight':'PASS_E1_E4','L5_3_full_E2':'PASS','L5_4_candidates':'PASS_E1_E4','L5_4_selection':'OPEN','L5_5':'WAIT_JOIN'}
