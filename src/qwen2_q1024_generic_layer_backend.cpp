...
static void run_attention(int layer,const std::string &output){
  constexpr int tokens=1024,heads=12,kv_heads=2,dim=128,hidden=1536,kv=256,block=128;
...
int main(int argc,char **argv){
  ...
  if(stage=="pre")run_pre(layer,input,predecessor,output);else if(stage=="attention")run_attention(layer,output);else if(stage=="oproj"){const auto probabilities=load<float>(output+"/attention_fp32.bin",size_t(tokens)*hidden);std::vector<uint16_t>mi(probabilities.size());
#pragma omp parallel for schedule(static)
