...
module tb_fp32_block32_softmax_tile16_candidate;
 ...
 task automatic run_case(input integer id);integer begin_cycle,mismatches;begin
  ...
  wr=0;if(mismatches)$fatal(1,"case=%0d mismatches=%0d",id,mismatches);if(flags[4:1]!=0||accepted!=512||issued!=128||completed!=128||reduced!=496)$fatal(1,"accounting case=%0d",id);while(busy)@(posedge clk);if(cycles-begin_cycle>max_case_cycles)max_case_cycles=cycles-begin_cycle;
 end endtask
