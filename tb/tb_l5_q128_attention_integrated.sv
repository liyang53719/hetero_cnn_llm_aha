...
module tb_l5_q128_attention_integrated;
 ...
 task automatic matrix_step(input logic[2:0]context_id,input logic clear_value,input logic[255:0]avec,input logic[511:0]bvec,output logic[16383:0]result);begin @(negedge clk);mcontext_i=context_id;mclear=clear_value;ma=avec;mb=bvec;minv=1;do@(posedge clk);while(!minr);@(negedge clk);minv=0;do@(posedge clk);while(!moutv);@(negedge clk);result=macc;if(merror||mflags[4:1]!=0)$fatal(1,"matrix protocol/flags");end endtask
 ...
 task automatic do_qk(input integer task_id,input integer qt,input integer kt,input integer qh,input integer kh,input integer rows);logic[16383:0]result;logic[255:0]avec;logic[511:0]bvec;integer qbase,kbase,slot;begin qbase=qt*16;kbase=kt*32;slot=task_id&1;for(integer dim=0;dim<128;dim++)begin avec='0;bvec='0;for(integer row=0;row<16;row++)if(row<rows)avec[row*16+:16]=qmem[qidx(qh,qbase+row,dim)];for(integer col=0;col<32;col++)if(kbase+col<seq_len)bvec[col*16+:16]=kmem[kidx(kh,kbase+col,dim)];matrix_step(3'd4,dim==0,avec,bvec,result);if(task_id==0&&(dim%32)==31)begin $display("INTEGRATED_PROGRESS phase=QK task=%0d step=%0d cycles=%0d",task_id,dim+1,cycles);$fflush();end end for(integer lane=0;lane<512;lane++)score_slot[slot][lane]=result[lane*32+:32];end endtask
 ...
 task automatic do_pv(input integer task_id,input integer qt,input integer kt,input integer qh,input integer kh,input integer rows,input integer close_block,input integer merge_global,input integer last_kv);logic[16383:0]result;logic[255:0]avec;logic[511:0]bvec;integer slot,kbase;begin
  ...
  for(integer term=0;term<2;term++)for(integer key=0;key<32;key++)for(integer group=0;group<4;group++)begin
   avec='0;bvec='0;
   for(integer row=0;row<16;row++)if(row<rows)avec[row*16+:16]=(term==0?weight_hi_slot[slot][row*32+key][31:16]:weight_lo_slot[slot][row*32+key][31:16]);
   for(integer col=0;col<32;col++)if(kbase+key<seq_len)bvec[col*16+:16]=vmem[kidx(kh,kbase+key,group*32+col)];
   matrix_step(group[2:0],term==0&&key==0,avec,bvec,result);
   if(task_id==0&&group==3&&key==31)begin $display("INTEGRATED_PROGRESS phase=PV task=%0d term=%0d cycles=%0d",task_id,term,cycles);$fflush();end
   if(term==1&&key==31)for(integer row=0;row<rows;row++)for(integer col=0;col<32;col++)tile_o[slot][row*128+group*32+col]=result[(row*32+col)*32+:32];
  end
