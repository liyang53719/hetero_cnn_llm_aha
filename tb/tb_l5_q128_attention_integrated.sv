...
module tb_l5_q128_attention_integrated;
 ...
 initial begin string vectors,captured_path;logic direct_diag,sampled_384,sampled_1024;logic full1024;integer full_qtile;integer sample_task,qt,qh,kvtiles,close_flag,merge_flag,last_flag,shard,sample_begin,sample_end;
  ...
  if(full1024)begin seq_len=1024;if(!$value$plusargs("QT=%d",full_qtile)||full_qtile<0||full_qtile>=64||direct_diag||sampled_384||sampled_1024)$fatal(1,"full1024 options");if(!$value$plusargs("OUTPUT=%s",captured_path))$fatal(1,"full1024 requires actual output path");captured_fd=$fopen(captured_path,"w");if(!captured_fd)$fatal(1,"capture open");end
  ...
  if(full1024)begin
   ...
   $fclose(captured_fd);captured_fd=0;
