...
module tb_qwen2_q1024_layer0_tail_rtl;
  ...
  logic[31:0]lfsr;logic[16383:0]last_matrix_acc;integer matrix_steps,matrix_values,matrix_output_count,silu_output_count;
  ...
  task automatic run_matrix(input string name,input integer ksteps);logic[16383:0]result;integer mismatches;begin
    $readmemh({"work/results/qwen2_q1024_layer0_tail_rtl/vectors/",name,"_activation.memh"},activation);
    $readmemh({"work/results/qwen2_q1024_layer0_tail_rtl/vectors/",name,"_weights.memh"},weights);
    $readmemh({"work/results/qwen2_q1024_layer0_tail_rtl/vectors/",name,"_expected.memh"},expected);
    ...
    if(mismatches)$fatal(1,"%s mismatches=%0d",name,mismatches);$display("QWEN2_LAYER0_MATRIX_SAMPLE_PASS operation=%s tiles=3 k=%0d values=1536",name,ksteps);
  end endtask
  initial begin logic[11:0]expected_tag;logic expected_last;clk=0;rst_n=0;minv=0;mor=0;mctx=0;mclear=0;mlast=0;ma=0;mb=0;siv=0;sgate=0;sup=0;stag_i=0;slast_i=0;matrix_steps=0;matrix_values=0;repeat(8)@(posedge clk);rst_n=1;
    run_matrix("oproj",1536);run_matrix("gate",1536);run_matrix("up",1536);run_matrix("down",8960);
    $readmemh("work/results/qwen2_q1024_layer0_tail_rtl/vectors/silu_gate8.memh",gate8);$readmemh("work/results/qwen2_q1024_layer0_tail_rtl/vectors/silu_up8.memh",up8);$readmemh("work/results/qwen2_q1024_layer0_tail_rtl/vectors/silu_product8.memh",product8);
    ...
    $display("QWEN2_Q1024_LAYER0_TAIL_RTL_PASS matrix_steps=40704 matrix_values=6144 silu_lanes=8 silu_values=8192 silu_random_backpressure=1 matrix_out_ready=1 same_RTL=1");$finish;
  end
