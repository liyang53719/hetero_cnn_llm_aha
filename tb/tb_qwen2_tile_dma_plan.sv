...
module tb_qwen2_tile_dma_plan;
 ...
 qwen2_tile_dma_plan dut(.clk_i(clk),.rst_ni(rst_n),.context_valid_i(cv),.context_ready_o(cr),
  .context_legal_i(cl),.tensor_address_i(addresses),.q_column_tile_i(6'd0),.weight_src_stride_i(32'd3072),.full_q_i(1'b0),.reuse_norm_i(1'b0),.start_store_i(store),.dma_req_valid_o(rv),
