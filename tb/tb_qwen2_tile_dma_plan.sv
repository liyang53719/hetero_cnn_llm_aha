...
module tb_qwen2_tile_dma_plan;
 ...
 qwen2_tile_dma_plan dut(.clk_i(clk),.rst_ni(rst_n),.context_valid_i(cv),.context_ready_o(cr),
  .context_legal_i(cl),.tensor_address_i(addresses),.q_column_tile_i(6'd0),.weight_src_stride_i(32'd3072),.token_index_i(1),.output_token_bytes_i(64),.full_q_i(1'b0),.reuse_norm_i(1'b0),.start_store_i(store),.dma_req_valid_o(rv),
 ...
 initial begin cv=0;cl=1;store=0;addresses=0;for(integer i=0;i<6;i++)begin a[i]=56'h100000+i*56'h100000;addresses[i*56+:56]=a[i];end
  ek[0]=0;es[0]={8'd0,a[0]}+3072;ed[0]=64'h40000;erb[0]=3072;erows[0]=1;ess[0]=3072;eds[0]=3072;
  ...
  ek[3]=2;es[3]=64'h5c000;ed[3]={8'd0,a[5]}+64;erb[3]=64;erows[3]=1;ess[3]=64;eds[3]=64;
  ...
  $display("QWEN2_TILE_DMA_PLAN_PASS requests=4 load1d=2 load2d=1 store1d=1 read_bytes=107520 write_bytes=64 q_weight_rows=1536 q_weight_row_bytes=64 src_stride=3072 dst_stride=64 random_backpressure=1 token_index=1 address_offset=1");$finish;end
