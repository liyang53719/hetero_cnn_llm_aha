...
#define main l2_conv3x3_reference_main_unused
...
int main(void){uint64_t checksum=0;
  ...
  for(int p=0;p<PIXELS;p++)for(int o=0;o<OUT_CH;o++)out_raw[0][p][o]=0;
  uint64_t start=read_cycles();raw_loop_conv(0);uint64_t end=read_cycles();
  for(int r=0;r<OUT_DIM;r++)for(int c=0;c<OUT_DIM;c++)for(int o=0;o<OUT_CH;o++){
    elem_t g=golden(0,r,c,o);int p=r*OUT_DIM+c;if(out_raw[0][p][o]!=g){
      printf("GEMMINI_L4_CONV3X3_IDENTITY_FAIL r=%d c=%d o=%d rtl=%d gold=%d\n",r,c,o,out_raw[0][p][o],g);exit(1);}
    checksum=checksum*131+(uint8_t)out_raw[0][p][o];}
  printf("GEMMINI_L4_CONV3X3_IDENTITY_PASS checksum=%lu cycles=%lu dma_bytes=299 macs=2700\n",checksum,end-start);exit(0);}
